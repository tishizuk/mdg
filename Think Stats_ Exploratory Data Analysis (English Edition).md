...
- SciPy for scientific computation including statistics StatsModels for regression and other statistical analysis 
...
After you clone the repository or unzip the zip file, you should have a file called `ThinkStats2/code/nsfg.py` . If you run it, it should read a data file, run some tests, and print a message like, “All tests passed.” If you get import errors, it probably means there are packages you need to install. 

Most exercises use Python scripts, but some also use the IPython notebook. If you have not used IPython notebook before, I suggest you start with the documentation. 

I wrote this book assuming that the reader is familiar with core Python, including object-oriented features, but not pandas, NumPy, and SciPy. If you are already familiar with these modules, you can skip a few sections. I assume that the reader knows basic mathematics, including logarithms, for example, and summations. I refer to calculus concepts in a few places, but you don’t have to do any calculus. 
...
- Tommie Gannert sent a patch file with a number of corrections. Alexander Gryzlov suggested a clarification in an exercise. 
...
- David Palmer sent many helpful suggestions and corrections. Erik Kulyk found many typos. 
...
We have a web page for this book, where we list errata, examples, and any additional information. You can access this page at http://bit.ly/think_stats_2e. 
...
If pregnancy length is longer for first babies, the difference is probably small compared to natural variation. In that case, we might have to compare a large number of pregnancies to be sure that a difference exists. Selection bias 
...
The NSFG is not representative; instead it is deliberately **oversampled** . The designers of the study recruited three groups — Hispanics, African Americans and teenagers — at rates higher than their representation in the U.S. population, in order to make sure that the number of respondents in each of these groups is large enough to draw valid statistical inferences. Of course, the drawback of oversampling is that it is not as easy to draw conclusions about the general population based on statistics from the survey. We will come back to this point later. 

When working with this kind of data, it is important to be familiar with the **codebook** , which documents the design of the study, the survey questions, and the encoding of the responses. The codebook and user’s guide for the NSFG data are available from  the CDC’s website. 

## **Importing the Data** 

The code  and data used in this book are available from GitHub. For information about downloading and working with this code, see Using the Code. 

Once you download the code, you should have a folder called `ThinkStats2/code` with a file called `nsfg.py` . If you run it, it should read a data file, run some tests, and print a message like, “All tests passed.” 
...
In addition to the data, a DataFrame also contains the variable names and their types, and it provides methods for accessing and modifying the data. If you print `df` you get a truncated view of the rows and columns, and the shape of the DataFrame, which is 13593 rows/records and 244 columns/variables. 
...
You can access the elements of a Series using integer indices and slices: `>>> pregordr[0] 1 >>> pregordr[2:5] 2    1 3    2 4    3 Name: pregordr, dtype: int64` 

The result of the index operator is an `int64` ; the result of the slice is another Series. 

You can also access the columns of a DataFrame using dot notation: `>>> pregordr = df.pregordr` 
...
- `birthord` is a serial number for live births; the code for a respondent’s first child is 1, and so on. For outcomes other than live births, this field is 
...
`birthwgt_lb` and `birthwgt_oz` contain the weight of the baby, in pounds and ounces, for pregnancies that end in live births. In addition they use several special codes: 
...
Special values encoded as numbers are _dangerous_ because if they are not handled properly, they can generate bogus results, like a 99-pound baby. The `replace` method replaces these values with `np.nan` , a special floatingpoint value that represents “not a number.” The `inplace` flag tells `replace` to modify the existing Series rather than create a new one. 
...
In the repository you downloaded, you should find a file named `chap01ex.ipynb` , which is an IPython notebook. You can launch IPython notebook from the command line like this: `$ ipython notebook &` 

If IPython is installed, it should launch a server that runs in the background and open a browser to view the notebook. If you are not familiar with IPython, I suggest you start at the IPython website. 
...
Open `chap01ex.ipynb` . Some cells are already filled in, and you should execute them. Other cells give you instructions for exercises you should try. A solution to this exercise is in `chap01soln.ipynb` 
...
You can use `nsfg.MakePregMap` to make a dictionary that maps from each `caseid` to a list of indices into the pregnancy DataFrame. A solution to this exercise is in `chap01soln.py` 

_Example 1-3._ 

The best way to learn about statistics is to work on a project you are interested in. Do you want to investigate a question like “Do first babies arrive late?” 
...
Two of my favorite data sets are the General Social Survey and the European Social Survey. 

If it seems like someone has already answered your question, look closely to see whether the answer is justified. There might be flaws in the data or the analysis that make the conclusion unreliable. In that case you could 

perform a different analysis of the same data, or look for a better source of data. 
...
Evidence, often personal, that is collected casually rather than by a welldesigned study. 
...
- Hist objects provide `Freq` , which takes a value and returns its frequency: `>>> hist.Freq(2) 2` 

- The bracket operator does the same thing: `>>> hist[2] 2` 

If you look up a value that has never appeared, the frequency is 0: `>>> hist.Freq(4) 0` 
...
- Or you can use `Items` to iterate through value-frequency pairs: `for val, freq in hist.Items(): print(val, freq)` 

## **Plotting Histograms** 

For this book I wrote a module called `thinkplot.py` that provides functions for plotting Hists and other objects defined in `thinkstats2.py` . It is based on `pyplot` , which is part of the `matplotlib` package. See Using the Code for information about installing `matplotlib` . 
...
Now let’s get  back to the data from the NSFG. The code in this chapter is in `first.py` . For information about downloading and working with this code, see Using the Code. 
...
In Transformation we transformed `agepreg` from centiyears to years, and combined `birthwgt_lb` and `birthwgt_oz` into a single quantity, `totalwgt_lb` . In this section I use these variables to demonstrate some features of histograms. 
...
The expression in brackets is a boolean Series that selects rows from the DataFrame and returns a new DataFrame. Next I generate and plot the 
...
Figure 2-3 shows the histogram of `agepreg` , the mother’s age at the end of pregnancy. The mode is 21 years. The distribution is very roughly bellshaped, but in this case the tail extends farther to the right than left; most mothers are in their 20s, fewer in their 30s. 
...
_Figure 2-1. Histogram of the pound part of birth weight_ 



_Figure 2-2. Histogram of the ounce part of birth weight_ 



_Figure 2-3. Histogram of mother’s age at end of pregnancy_ 



_Figure 2-4. Histogram of pregnancy length in weeks_ 
...
In the list of pregnancy lengths for live births, the 10 lowest values are `[0, 4, 9, 13, 17, 18, 19, 20, 21, 22]` . Values below 10 weeks are certainly errors; the most likely explanation is that the outcome was not coded correctly. Values higher than 30 weeks are probably legitimate. 

Between 10 and 30 weeks, it is hard to be sure; some values are probably errors, but some represent premature babies. 
...
`thinkplot.PrePlot` takes the number of histograms we are planning to plot; it uses this information to choose an appropriate collection of colors. `thinkplot.Hist` normally uses `align=’center’` so that each bar is centered over its value. For this figure, I use `align=’right’` and 

`align=’left’` to place corresponding bars on either side of the value. With `width=0.45` , the total width of the two bars is 0.9, leaving some space between each pair. Finally, I adjust the axis to show only data between 27 and 46 weeks. Figure 2-5 shows the result. 



_Figure 2-5. Histogram of pregnancy lengths._ 
...
If you have a sample of `n` values, xi, the mean, , is the sum of the values divided by the number of values: 



The words _mean_ and _average_ are sometimes used interchangeably, but I make this distinction: 

- The _mean_ of a sample  is the summary statistic computed with the previous formula. 

An _average_ is one  of several summary statistics you might choose to describe a central tendency. 

Sometimes the mean is a good description of a set of values. For example, apples are all pretty much the same size (at least the ones sold in supermarkets). So if I buy 6 apples and the total weight is 3 pounds, it would be a reasonable summary to say they are about a half pound each. But pumpkins are more diverse.  Suppose I grow several varieties in my garden, and one day I harvest three decorative pumpkins that are 1 pound each, two pie pumpkins that are 3 pounds each, and one Atlantic Giant pumpkin that weighs 591 pounds. The mean of this sample is 100 pounds, but if I told you “The average pumpkin in my garden is 100 pounds,” that would be misleading. In this example, there is no meaningful average because there is no typical pumpkin. 
...
If you have prior experience, you might have seen a formula for variance with in the denominator, rather than `n` . This statistic is used to estimate the variance in a population using a sample. We will come back to this in Chapter 8. 
...
Variance of pregnancy length is 7.3, which is hard to interpret, especially since the units are weeks<sup>2</sup> , or “square weeks.” Variance is useful in some calculations, but it is not a good summary statistic. 
...
If we assume this estimate is accurate, such a difference would have no practical consequences. In fact, without observing a large number of pregnancies, it is unlikely that anyone would notice this difference at all. Another way to convey the size of the effect is to compare the difference between groups to the variability within groups. Cohen’s d is a statistic intended to do that; it is defined 



where and are the means of the groups and s is the “pooled standard deviation”. Here’s the Python code that computes Cohen’s d: 
...
Of course your decisions should also be guided by  professional ethics. It’s OK to be persuasive; you _should_ design statistical reports and visualizations that tell a story clearly. But you should also do your best to make your reports honest, and to acknowledge uncertainty and limitations. 
...
Finally, imagine that you are Cecil Adams, author of _The Straight Dope_ , and your job is to answer the question, “Do first babies arrive late?” Write a paragraph that uses the results in this chapter to answer the question clearly, precisely, and honestly. 
...
The mode of a distribution is the most frequent value; see Wikipedia. Write a function called `Mode` that takes a Hist and returns the most frequent value. As a more challenging exercise, write a function called `AllModes` that returns a list of value-frequency pairs in descending order of frequency. 
...
_Figure 3-1. PMF of pregnancy lengths for first babies and others, using bar graphs and step functions_ 
...
By plotting the PMF instead of the histogram, we can compare the two distributions without being misled by the difference in sample size. Based on this figure, first babies seem to be less likely than others to arrive on time (week 39) and more likely to be late (weeks 41 and 42). 
...
_Figure 3-2. Difference, in percentage points, by week_ 
...
But if you survey a group of students, ask them how many students are in their classes, and compute the mean, you would think the average class size was bigger. Let’s see how much bigger. 
...
Figure 3-3 shows the result. In the biased distribution there are fewer small classes and more large ones. The mean of the biased distribution is 29.1, 

almost 25% higher than the actual mean. 



_Figure 3-3. Distribution of class sizes, actual and as observed by students_ 
...
In DataFrames we read a  pandas DataFrame and used it to select and modify data columns. Now let’s look at row selection. To start, I create a NumPy array of random numbers and use it to initialize a DataFrame: 
...
`loc` can also take a list of labels; in that case, the result is a DataFrame. `>>> indices = ['a', 'c'] >>> df.loc[indices] A         B a -0.14351  0.616050 c -0.07435  0.039621` 
...
Now compute the biased distribution we would see if we surveyed the children and asked them how many children under 18 (including 
...
where the xi are the unique values in the PMF and . Similarly, you can compute variance like this: 
...
I started with the question, “Are first babies more likely to be late?” To address it, I computed the difference in means between groups of babies, but I ignored the possibility that there might be a difference between first babies and others _for the same woman_ . 
...
The code for this chapter is in `cumulative.py` . For information about downloading and working with this code, see Using the Code. 
...
_Figure 4-1. PMF of birth weights showing a limitation of PMFs: they are hard to compare visually_ 
...
The CDF is a function of x, where x is any value that might appear in the distribution. To evaluate for a particular value of x, we compute the fraction of values in the distribution less than or equal to x. Here’s what that looks like as a function that takes a sequence, `t` , and a value, `x` : 
...
We can evaluate the CDF for any value of x, not just values that appear in the sample. If x is less than the smallest value in the sample, is 0. If x is greater than the largest value, is 1. Figure 4-2 is a graphical representation of this CDF. The CDF of a sample is a step function. 



_Figure 4-2. Example of a CDF_ 
...
Given a value `x` , computes the probability . The bracket operator is equivalent to `Prob` . 
...
_Figure 4-3. CDF of pregnancy length_ 
...
CDFs are especially useful for comparing distributions. For example, here is the code that plots the CDF of birth weights for first babies and others. 
...
_Figure 4-4. CDF of birth weights for first babies and others_ 

## **Percentile-Based Statistics** 

Once you have computed a CDF, it is easy to compute percentiles and percentile ranks. The Cdf class provides these two methods: `PercentileRank(x)` 

. Given a value `x` , computes its percentile rank, `Percentile(p)` 
...
Here’s how we can compute it. First, we  make the Cdf of birth weights: `weights = live.totalwgt_lb` 
...
_Figure 4-5. CDF of percentile ranks for a random sample of birth weights_ 

That outcome might not be obvious, but it is a consequence of the way the CDF is defined. What this figure shows is that 10% of the sample is below the 10th percentile, 20% is below the 20th percentile, and so on, exactly as we should expect. 
...
- Choose a percentile rank uniformly from the range 0–100. Use `Cdf.Percentile` to find the value in the distribution that corresponds to the percentile rank you chose. 
...
A few years ago I ran the James Joyce Ramble 10K in Dedham MA; I finished in 42:44, which was 97th in a field of 1633. I beat or tied 1537 runners out of 1633, so my percentile rank in the field was 94%. 
...
In my age group, denoted M4049 for “males between 40 and 49 years of age”, I came in 26th out of 256. So my percentile rank in my age group was 90%. 
...
- A function that maps  from values to their cumulative probabilities. is the fraction of the sample less than or equal to x. 
...
_Figure 5-1. CDFs of exponential distributions with various parameters._ 

As an example, we will look at the interarrival time of births. On December 18, 1997, 44 babies were born in a hospital in Brisbane, Australia.<sup>[</sup> <u>1</u><sup>]</sup> The time of birth for all 44 babies was reported in the local paper; the complete dataset is in a file called `babyboom.dat` , in the `ThinkStats2` repository. 
...
`diffs` is the difference between consecutive birth times, and `cdf` is the distribution of these interarrival times. Figure 5-2 (left) shows the CDF. It 

seems to have the general shape of an exponential distribution, but how can we tell? 



_Figure 5-2. CDF of interarrival times (left) and CCDF on a log-y scale (right)_ 

One way is to plot the **complementary CDF** , which is , on a log-y scale. For data from an exponential distribution, the result is a straight line. Let’s see why that works. 
...
So on a log-y scale the CCDF is a straight line with slope . Here’s how we can generate a plot like that: 
...
The parameter, λ, can be interpreted as a rate; that is, the number of events that occur, on average, in a unit of time. In this example, 44 babies are born in 24 hours, so the rate is births per minute. The mean of an exponential distribution is , so the mean time between  births is 32.7 minutes. 
...
This result is correct: the median of the standard normal distribution is 0 (the same as the mean), and half of the values fall below the median, so is 0.5. 
...
_Figure 5-3. CDF of normal distributions with a range of parameters._ 
...
2. From a standard normal distribution ( and ), generate a random sample with the same size as the sample, and sort it. 
...
_Figure 5-5. Normal probability plot for random samples from normal distributions_ 
...
`FitLine` takes a sequence of `xs` , an intercept, and a slope; it returns `xs` and `ys` that represent a line with the given parameters, evaluated at the values in 
...
_Figure 5-6. Normal probability plot of birth weights_ 

When we select only full term births, we remove some of the lightest weights, which reduces the discrepancy in the lower tail of the distribution. This plot suggests that the normal model describes the distribution well within a few standard deviations from the mean, but not in the tails. 

Whether it is good enough for practical purposes depends on  the purposes. **The lognormal Distribution** 

If the logarithms of a  set of values have a normal distribution, the values have a **lognormal distribution** . The CDF of the lognormal distribution is the same as the CDF of the normal distribution, with substituted for x. 



The parameters of the lognormal distribution are usually denoted μ and σ. But remember that these parameters are _not_ the mean and standard deviation; the mean of a lognormal distribution is and the standard deviation is ugly (see Wikipedia). 
...
As an example, let’s look at the distribution of adult weights, which is approximately lognormal.<sup>[</sup> <u>2</u><sup>]</sup> 

The National Center for Chronic Disease Prevention and Health Promotion conducts an annual survey as part of the Behavioral Risk Factor Surveillance System (BRFSS).<sup>[</sup> <u>3</u><sup>]</sup> In 2008, they interviewed 414,509 respondents and asked about their demographics, health, and health risks. Among the data they collected are the weights in kilograms of 398,484 respondents. 
...
_Figure 5-7. CDF of adult weights on a linear scale (left) and log scale (right)_ 

Figure 5-8 shows normal probability plots for adult weights, w, and for their logarithms, . Now it is apparent that the data deviate substantially from the normal model. The lognormal model is a good match for the data  within a few standard deviations of the mean, but it deviates in the tails. I conclude that the lognormal distribution is a good model for this data. 



_Figure 5-8. Normal probability plots for adult weight on a linear scale (left) and log scale (right)._ 
...
The parameters xm and α determine the location and shape of the distribution. xm is the minimum possible value. Figure 5-9 shows CDFs of Pareto distributions with and different values of α. 



_Figure 5-9. CDFs of Pareto distributions with different parameters._ 
...
versus So if you plot , it should look like a straight line with slope . and intercept 
...
I downloaded their data; it is in the repository for this book in a file named `PEP_2012_PEPANNRES_with_ann.csv` . The repository also contains `populations.py` , which reads the file and plots the distribution of populations. 

Figure 5-10 shows the CCDF of  populations on a log-log scale. The largest 1% of cities and towns, below , fall along a straight line. So we could conclude, as some researchers have, that the tail of this distribution fits a Pareto model. 



_Figure 5-10. CCDFs of city and town populations, on a log-log scale_ 
...
_Figure 5-11. CDF of city and town populations on a log-x scale (left), and normal probability plot of log-transformed populations (right)_ 

## **Generating Random Numbers** 

Analytic CDFs can be used to generate random numbers with a given distribution function, . If there is an efficient way to compute the inverse CDF, we can generate random values with the appropriate distribution by choosing p from a uniform distribution between 0 and 1, and . then choosing 
...
Two notes about this implementation: I called the parameter `lam` because `lambda` is a Python keyword. Also, since is undefined, we have to be a little careful. The  implementation of `random.random` can return 0 but not 1, so can be 1 but not 0, so `log(1-p)` is always  defined. 
...
In the BRFSS (see The lognormal Distribution), the distribution of heights is roughly normal with parameters cm and cm for men, and cm and cm for women. 
...
To get a feel for the Pareto distribution, let’s see how different the world would be if the distribution of human height were Pareto. With the parameters m and , we get a distribution with a reasonable minimum, 1 m, and median, 1.5 m. 

Plot this distribution. What is the mean human height in Pareto world? What fraction of the population is shorter than the mean? If there are 7 billion people in Pareto world, how many do we expect to be taller than 1 km? How tall do we expect the tallest person to be? 

###### _Example 5-3._ 

The Weibull distribution is a generalization of the exponential distribution that comes up in failure analysis. Its CDF is: 



Can you find a transformation that makes a Weibull distribution look like a straight line? What do the slope and intercept of the line indicate? Use `random.weibullvariate` to generate a sample from a Weibull distribution and use it to test your transformation. 
...
You will also find `test_models.py` , a script that reads data from a file and plots the CDF under a variety of transforms. You can run it like this: `$ python test_models.py mystery0.dat` 
...
The Current Population Survey (CPS) is a joint effort of the Bureau of Labor Statistics and the Census Bureau to study income and related variables. Data collected in 2013 is available from the Census Bureau. I 

downloaded `hinc06.xls` , which is an Excel spreadsheet with information about household income, and converted it to `hinc06.csv` , a CSV file you will find in the repository for this book. You will also find `hinc.py` , which reads this file. 
...
A function that maps from a value, x, to the fraction of values that exceed x, which is . 
...
[1<sup>]</sup> This example is based on information and data from Dunn, “A Simple Dataset for Demonstrating Common Distributions,” Journal of Statistics Education v.7, n.3 (1999). 

[2<sup>]</sup> I was tipped off to this possibility by a comment (without citation). Subsequently I found a paper that proposes the log transform and suggests a cause: Penman and Johnson, “The Changing Shape of the Body Mass Index Distribution Curve in the Population,” Preventing Chronic Disease, 2006 July; 3(3): A74. 

[3<sup>]</sup> Centers for Disease Control and Prevention (CDC). Behavioral Risk Factor Surveillance System Survey Data. Atlanta, Georgia: US Department of Health and Human Services, Centers for Disease Control and Prevention, 2008. 
...
Evaluating a PDF for a particular value of x is usually not useful. The result is not a probability; it is a probability _density_ . 
...
The NormalPdf object contains the parameters `mu` and `sigma` . `Density` uses `scipy.stats.norm` , which is  an object that represents a normal distribution and provides `cdf` and `pdf` , among other methods (see The Normal Distribution). 

The following example creates a NormalPdf with the mean and variance of adult female heights, in cm, from the BRFSS (see The lognormal Distribution). Then it computes the density of the distribution at a location one standard deviation from the mean. 
...
`thinkplot.Pdf` plots the Pdf as a smooth function, as contrasted with `thinkplot.Pmf` , which renders a Pmf as a step  function. Figure 6-1 shows 

the result, as well as a PDF estimated from a sample, which we’ll compute in the next section. 



_Figure 6-1. A normal PDF that models adult female height in the US, and the kernel density estimate of a sample with ._ 
...
**Kernel density estimation** (KDE) is an  algorithm that takes a sample and finds an appropriately smooth PDF that fits the data. You can read details at Wikipedia. 
...
`Density` takes a value or sequence, calls `gaussian_kde.evaluate` , and returns the resulting density. The word “Gaussian” appears in the name because it uses a filter based on a Gaussian distribution to smooth the KDE. Here’s an example that generates a sample from a normal distribution and then makes an EstimatedPdf to  fit it: 
...
Estimating a density function with KDE is useful for several purposes: Visualization 
...
_Figure 6-2. A framework that relates representations of distribution functions_ 
...
A CDF maps from values to  cumulative probabilities, so I could have implemented Cdf as a `_DictWrapper` . But the values in a CDF are ordered and the values in a `_DictWrapper` are not. Also, it is often useful to compute the inverse CDF; that is, the map from cumulative probability to value. So the implementaion I chose is two sorted lists. That way I can use binary search to do a forward or inverse lookup in  logarithmic time. 

The Cdf constructor can take as a parameter a sequence of values or a pandas Series, a dictionary that maps from values to probabilities, a sequence of (value, probability) pairs, a Hist, Pmf, or Cdf. Or if it is given two parameters, it treats them as a sorted sequence of values and the sequence of corresponding cumulative probabilities. Given a sequence, pandas Series, or dictionary, the constructor makes a Hist. Then it uses the Hist to initialize the attributes: 
...
`xs` is the sorted list of values; `freqs` is the list of corresponding frequencies. `np.cumsum` computes the cumulative sum of the frequencies. Dividing through by the total frequency yields cumulative probabilities. For `n` values, . the time to construct the Cdf is proportional to 
...
When the result is the sample mean, . The other raw moments don’t mean much by themselves, but they are used in some computations. The **central moments** are  more useful. The kth central moment is: 
...
When the result is the second central moment, which you might recognize as variance. The definition of variance gives a hint about why 

these statistics are called moments. If we attach a weight along a ruler at each location, xi, and then spin the ruler around the mean, the moment of inertia of the spinning weights is the variance of the values. If you are not familiar with moment of inertia, see Wikipedia. . 

When you report moment-based statistics, it is important to think about the units. For example, if the values xi are in cm, the first raw moment is also in cm. But the second moment is in cm<sup>2</sup> , the third moment is in cm<sup>3</sup> , and so on. 
...
Where is the sample mean, m is the median, and S is the standard deviation. Or in Python: 
...
_Figure 6-3. Estimated PDF of birthweight data from the NSFG_ 
...
_Figure 6-4. Estimated PDF of adult weight data from the BRFSS._ 
...
It also takes `log_upper` , which is an assumed upper bound on the highest range, expressed in `log10` dollars. The default value, `log_upper=6.0` represents the assumption that the largest income among the respondents is 10<sup>6</sup> , or one million dollars. 
...
_Figure 7-1. Scatter plots of weight versus height for the respondents in the BRFSS, unjittered (left), jittered (right)._ 
...
The values can be any sequence; the result is a NumPy array. Figure 7-1 (right) shows the result. Jittering reduces the visual effect of rounding and makes the shape of the relationship clearer. But in general you should only jitter data for purposes of visualization and avoid using jittered data for analysis. 
...
_Figure 7-2. Scatter plot with jittering and transparency (left), hexbin plot (right)._ 

Using transparency works well for moderate-sized datasets, but this figure only shows the first 5000 records in the BRFSS, out of a total of 414,509. To handle larger datasets, another option is a hexbin plot, which divides the graph into hexagonal bins and colors each bin according to how many data points  fall in it. `thinkplot` provides `HexBin` : 

```
thinkplot.HexBin(heights, weights)
```

Figure 7-2 (right) shows the result. An advantage of a hexbin is that it shows the shape of the relationship well, and it is efficient for large 

datasets, both in time and in the size of the file it generates. A drawback is that it makes the outliers invisible. 
...
`dropna` drops rows with `nan` in any of the listed columns. `arange` makes a NumPy array of bins from 135 to, but not including, 210, in increments of 5. 
...
`groupby` is a DataFrame method that returns a GroupBy object; used in a `for` loop, `groups` iterates the names of the groups and the DataFrames that represent them. So, for example, we can print the number of rows in each group like this: 
...
Figure 7-3 shows the result. Between 140 and 200 cm the relationship between these variables is roughly linear. This range includes more than 99% of the data, so we don’t have to worry too much about the extremes. 



_Figure 7-3. Percentiles of weight for a range of height bins._ 
...
If X is a series of n values, xi, we can convert to standard scores by . subtracting the mean and dividing by the standard deviation: The numerator is a deviation: the distance from the mean. Dividing by σ **standardizes** the deviation, so the values of Z are dimensionless (no units) and their distribution has mean 0 and variance 1. 
...
where is the sample mean of X and is the sample mean of Y. If X and Y vary together, their deviations tend to have the same sign. 
...
If Pearson’s correlation is near 0, it is  tempting to conclude that there is no relationship between the variables, but that conclusion is not valid. Pearson’s correlation only measures _linear_ relationships. If there’s a nonlinear relationship, ρ understates its strength. 

Figure 7-4 is from the Wikipedia page for correlation and dependence. It shows scatter plots and correlation coefficients for several carefully constructed datasets. 



_Figure 7-4. Examples of datasets with a range of correlations._ 
...
I could also use `Series.corr` directly and specify Spearman’s method: `def SpearmanCorr(xs, ys): xranks = pandas.Series(xs) yranks = pandas.Series(ys) return xs.corr(ys, method='spearman')` 
...
- If the relationship is nonlinear, Pearson’s correlation tends to underestimate the strength of  the relationship. Pearson’s correlation can be affected (in either direction) if one of the distributions is skewed or contains outliers. Spearman’s rank correlation is more robust. 

In the BRFSS example, we know that the distribution of weights is roughly lognormal; under a log transform, it approximates a normal distribution, so 

it has no skew. So another way to eliminate the effect of skewness is to compute Pearson’s correlation with log-weight and height: 
...
- Use randomness. If you divide a large sample into two groups at random and compute the means of almost any variable, you expect the difference to be small. If the groups are nearly identical in all variables but one, you can eliminate spurious relationships. 
...
Unfortunately, controlled trials are only possible in the laboratory sciences, medicine, and a few other disciplines. In the social sciences, controlled experiments are rare, usually because they are impossible or unethical. An alternative is to look for a **natural experiment** , where different “treatments” are applied to groups that are otherwise similar. One danger of natural experiments is that the groups might differ in ways that are not apparent. 
...
A visualization of the relationship between two variables, showing one point for each row of data. 

jitter 

Random noise added to data for purposes of visualization. saturation 

Loss of information when multiple points are plotted on top of each other. correlation 
...
A value that has been standardized so that it is expressed in standard deviations from the mean. 
...
An experimental design in which subjects are divided into groups at random, and different groups are given different  treatments. 
...
- control group 
   ...
   - A group in a controlled trial that receives no treatment, or a treatment whose effect is  known. 

###### natural experiment 

An experimental design that takes advantage of a natural division of subjects into groups in ways that are at least approximately random. 
...
_I’m thinking of a distribution._ It’s a normal distribution, and here’s a sample that was collected by an unreliable surveyor who occasionally puts the decimal point in the wrong  place. 
...
One option is to identify and  discard outliers, and then compute the sample mean of the rest. Another option is to use the median as an estimator. Which estimator is best depends on the circumstances (for example, whether there are outliers) and on what the goal is. Are you trying to minimize errors, or maximize your chance of getting the right answer? If there are no outliers, the sample mean minimizes the **mean squared error** (MSE). That is, if we play the game many times, and each time compute the error , the sample mean minimizes 



Where m is the number of times you play the estimation game, not to be . confused with n, which is the size of the sample used to compute Here is a function that simulates the estimation game and computes the root mean squared error (RMSE), which is the square root of MSE: 
...
Again, `n` is the size of the sample, and `m` is the number of times we play the game. `means` is the list of estimates based on . `medians` is the list of medians. 
...
When I ran this code, the RMSE of the sample mean was 0.41, which means that if we use to estimate the mean of this distribution, based on a sample with , we should expect to be off by 0.41 on average. Using the median to estimate the mean yields RMSE 0.53, which confirms that yields lower RMSE, at least for this example. 
...
_I’m thinking of a distribution._ It’s a  normal distribution, and here’s a (familiar) sample: 

```
[-0.441, 1.774, -0.101, -1.138, 2.975, -2.138]
```

What do you think is the variance, σ<sup>2</sup> , of my distribution? Again, the obvious choice is to use the sample variance, S<sup>2</sup> , as an estimator. 



For large samples, S<sup>2</sup> is an adequate estimator, but for small samples it tends to be too low. Because of this unfortunate property, it is called a **biased** estimator. An estimator is **unbiased** if the expected total (or mean) error, after many iterations of the estimation game, is 0. Fortunately, there is another simple statistic that is an unbiased estimator of σ<sup>2</sup> : 



For an explanation of why S<sup>2</sup> is biased, and a proof that is unbiased, see this Wikipedia page. 

The biggest problem with this estimator is that its name and symbol are used inconsistently. The name “sample variance” can refer to either S<sup>2</sup> or 



, and the symbol S<sup>2</sup> is used for either or both. Here is a function that simulates the estimation game and tests the : performance of S<sup>2</sup> and `def Estimate2(n=7, m=1000): mu = 0 sigma = 1 estimates1 = [] estimates2 = [] for _ in range(m): xs = [random.gauss(mu, sigma) for i in range(n)] biased = np.var(xs) unbiased = np.var(xs, ddof=1) estimates1.append(biased) estimates2.append(unbiased) print('mean error biased', MeanError(estimates1, sigma**2)) print('mean error unbiased', MeanError(estimates2, sigma**2))` 

Again, `n` is the sample size and `m` is the number of times we play the game. `np.var` computes S<sup>2</sup> by default and if you provide the argument `ddof=1` , which stands for “delta degrees of freedom.” I won’t explain that term, but you can read about it at Wikipedia. 
...
When I ran this code, the mean error for S<sup>2</sup> was -0.13. As expected, this biased estimator tends to be too low. For , the mean error was 0.014, about 10 times smaller. As `m` increases, we expect the mean error for to approach 0. 
...
Suppose you are a  scientist studying gorillas in a wildlife preserve. You want to know the average weight of the adult female gorillas in the preserve. To weigh them, you have to tranquilize them, which is dangerous, expensive, and possibly harmful to the gorillas. But if it is important to obtain this information, it might be acceptable to weigh a sample of 9 gorillas. Let’s assume that the population of the preserve is well known, so we can choose a representative sample of adult females. We could use the sample mean, , to estimate the unknown population mean, μ. Having weighed 9 female gorillas,  you might find kg and sample standard deviation, kg. The sample mean is an unbiased estimator of μ, and in the long run it minimizes MSE. So if you report a single estimate that summarizes the results, you would report 90 kg. 

But how confident should you be in this estimate? If you only weigh gorillas out of a much larger population, you might be unlucky and choose the 9 heaviest gorillas (or the 9 lightest ones) just by chance. Variation in the estimate caused by random selection is called **sampling error** . To quantify sampling error, we can simulate the sampling process with hypothetical values of μ and σ, and see how much varies. 

Since we don’t know the actual values of μ and σ in the population, we’ll use the estimates and S. So the question we answer is: “If the actual values of μ and σ were 90 kg and 7.5 kg, and we ran the same experiment many times, how much would the estimated mean, , vary?” 
...
`mu` and `sigma` are the _hypothetical_ values of the parameters. `n` is the sample size, the number of gorillas we measured. `m` is the number of times we run the simulation. 

In each iteration, we choose `n` values from a normal distribution with the given parameters, and compute the sample mean, `xbar` . We run 1000 simulations and then compute the distribution, `cdf` , of the estimates. The result is shown in Figure 8-1. This distribution is called the **sampling distribution** of the estimator. It shows how much the estimates would vary if we ran the experiment over and over. 

The mean of the sampling distribution is pretty close to the hypothetical value of μ, which means that the experiment yields the right answer, on average. After 1,000 tries, the lowest result is 82 kg, and the highest is 98 kg. This range suggests that the estimate might be off by as much as 8 kg. 



_Figure 8-1. Sampling distribution of , with confidence interval_ 

There are two common ways to summarize the sampling distribution: **Standard error** (SE) 

A measure of how far we expect the estimate to be off, on average. For each simulated experiment, we compute the error, , and then compute the **root mean squared error** (RMSE). In this example, it is roughly 2.5 kg. 

- **Confidence interval** (CI) 

   - A range that includes a given fraction of the sampling distribution. For example, the 90% confidence interval is the range from the 5th to the 95th percentile. In this example, the 90% CI is kg. 
...
- Standard errors and confidence intervals are the source of much confusion: 
   ...
   - People often think that there is a 90% probability that the actual parameter, μ, falls in the 90% confidence interval. Sadly, that is not true. If you want to make a claim like that, you have to use Bayesian methods (see my book, _Think Bayes_ ). 
...
Let’s play one more round of the estimation game. _I’m thinking of a distribution._ It’s an exponential distribution, and here’s a sample: `[5.384, 4.493, 19.198, 2.790, 6.122, 12.844]` 

What do you think is the parameter, λ, of this distribution? In general, the mean of an exponential distribution is , so working backwards, we might choose 



L is an estimator of λ. And not just any estimator; it is also the maximum likelihood estimator. So if you want to maximize your chance of guessing λ exactly, L is the way to go. But we know that is not robust in the presence of outliers, so we expect L to have the same problem. 

We can choose an alternative based on the sample median. The median of an exponential distribution is , so working backwards again, we can define an estimator 
...
When I run this experiment with , the RMSE of L is 1.1. For the median-based estimator Lm, RMSE is 1.8. We can’t tell from this experiment whether L minimizes MSE, but at least it seems better than Lm. Sadly, it seems that both estimators are biased. For L the mean error is 0.33, and for Lm it is 0.45. And neither converges to 0 as `m` increases. 

It turns out that is an unbiased estimator of the mean of the distribution, , but L is not an unbiased estimator of λ. 
...
In this chapter we used and median to estimate μ, and found that yields lower MSE. Also, we used S<sup>2</sup> and to estimate σ, and found that S<sup>2</sup> is biased and is unbiased. Run similar experiments to see if and median are biased estimates of μ. Also check whether S<sup>2</sup> or yields a lower MSE. 

###### _Example 8-2._ 

Suppose that you draw a sample with size from an exponential distribution with . Simulate this experiment 1000 times and plot the sampling distribution of the estimate L. Compute the standard error of the estimate and the 90% confidence interval. 

Repeat the experiment with a few different values of n and make a plot of standard error versus n. 

_Example 8-3._ 

In games like hockey and soccer, the time between goals is roughly exponential. So you could estimate a team’s goal-scoring rate by observing the number of goals they score in a game. This estimation process is a little different from sampling the time between goals, so let’s see how it works. Write a function that takes a goal-scoring rate, `lam` , in goals per game, and simulates a game by generating the time between goals until the total time exceeds one game, then returns the number of goals scored. 
...
The process of inferring the  parameters of a distribution from a sample. estimator 
...
- The square root of MSE, a more meaningful representation of typical error magnitude. 

maximum likelihood estimator (MLE) 

An estimator that computes the point estimate most likely to be correct. bias (of an estimator) 
...
The code for this chapter is in `hypothesis.py` . For information about downloading and working with this code, see Using the Code. 
...
There are several ways we could formulate this question, including Fisher null hypothesis testing, Neyman-Pearson decision theory, and Bayesian inference.<sup>[</sup> <u>4</u><sup>]</sup> What I present here is a subset of all three that makes up most of what people use in practice, which I will call **classical hypothesis testing** . 
...
- The first step is to quantify the size of the apparent effect by choosing a **test statistic** . In the NSFG example, the apparent effect is a difference in pregnancy length between first babies and others, so a natural choice for the test statistic is the difference in means between the two groups. The second step is to define a **null hypothesis** , which is a model of the system based on the assumption that the apparent effect is not real. In the NSFG example the null hypothesis is that there is no difference between first babies and others; that is, that pregnancy lengths for both groups have the same distribution. 

- The third step is to compute a **p-value** , which is the probability of seeing the apparent effect if the null hypothesis is true. In the NSFG example, we would compute the actual difference in means, then compute the 

probability of seeing a difference as big, or bigger, under the null hypothesis. 
...
`HypothesisTest` is an abstract parent class that provides complete definitions for some methods and place-keepers for others. Child classes 

based on `HypothesisTest` inherit `__init__` and `PValue` and provide `TestStatistic` , `RunModel` , and optionally `MakeModel` . 
...
As a simple example,<sup>[</sup> <u>5</u><sup>]</sup> suppose we toss a coin 250 times and see 140 heads and 110 tails. Based on this result, we might suspect that the coin is biased; that is, more likely to land heads. To test this hypothesis, we compute the probability of seeing such a difference if the coin is actually fair: 
...
The parameter, `data` , is a pair of integers: the number of heads and tails. The test statistic is the absolute difference between them, so `self.actual` is 30. 
...
`RunModel` simulates the null hypothesis by shuffling the pooled values and splitting them into two groups with sizes `n` and `m` . As always, the return value from `RunModel` has the same format as the observed data. To test the difference in pregnancy length, we run: 
...
If we run the same analysis with birth weight, the computed p-value is 0; after 1000 attempts, the simulation never yields an effect as big as the observed difference, 0.12 lbs. So we would report , and conclude that the difference in birth weight is statistically significant. 
...
_Figure 9-1. CDF of difference in mean pregnancy length under the null hypothesis_ 
...
`DiffMeansPermute` ; the only difference is that `TestStatistic` does not take the absolute value of the difference. This kind of test is called **one-sided** because it only counts one side of the distribution of differences. The previous test, using both sides, is **two-sided** . 

For this version of the test, the p-value is 0.09. In general, the p-value for a one-sided test is about half the p-value for a two-sided test, depending on 
...
We can use the same framework to test for a difference in standard deviation. In Other Visualizations, we saw some evidence that first babies are more likely to be early or late, and less likely to be on time. So we might hypothesize that the standard deviation is higher. Here’s how we can test that: 
...
To test this hypothesis, we can compute the expected frequency for each value, the difference between the expected and observed frequencies, and the total absolute difference. In this example, we expect each side to come up 10 times out of 60; the deviations from this expectation are -2, -1, 9, -5, 

-2, and 1; so the total absolute difference is 20. How often would we see such a difference by chance? 
...
The p-value using the chi-squared statistic is 0.04, substantially smaller than what we got using total deviation, 0.13. If we take the 5% threshold seriously, we would consider this effect statistically significant. But considering the two tests together, I would say that the results are borderline. I would not rule out the possibility that the die is crooked, but I would not convict the accused  cheater. 
...
Earlier in this chapter  we looked at pregnancy lengths for first babies and others, and concluded that the apparent differences in mean and standard deviation are not statistically significant. But in Other Visualizations, we saw several apparent differences in the distribution of pregnancy length, especially in the range from 35 to 43 weeks. To see whether those differences are statistically significant, we can use a test based on a chisquared statistic. 
...
For the NSFG data the total chi-squared statistic is 102, which doesn’t mean much by itself. But after 1,000 iterations, the largest test statistic generated under the null hypothesis is 32. We conclude that the observed chi-squared statistic is unlikely under the null hypothesis, so the apparent effect is statistically significant. 
...
- If the effect is actually due to chance, what is the probability that we will wrongly consider it significant? This probability is the **false positive rate** . If the effect is real, what is the chance that the hypothesis test will fail? This probability is the **false negative rate** . 
...
- Each time we run an experiment, we get a test statistic, t, which is drawn from CDFT. Then we compute a p-value, which is the probability that a . 

- random value from CDFT exceeds `t` , so that’s 

- The p-value is less than 5% if is greater than 95%; that is, if t exceeds the 95th percentile. And how often does a value chosen from CDFT exceed the 95th percentile? 5% of the time. 
...
First, I performed multiple tests. If you run one hypothesis test, the chance of a false positive is about 1 in 20, which might be acceptable. But if you run 20 tests, you should expect at least one false positive, most of the time. Second, I used the same dataset for exploration and testing. If you explore a large dataset, find a surprising effect, and then test whether it is significant, you have a good chance of generating a false positive. 

To compensate for multiple tests, you can adjust the p-value threshold (see this Wikipedia page). Or you can address both problems by partitioning the data, using one set for exploration and the other for testing. 
...
- The difference in mean pregnancy length is 0.16 weeks and statistically significant with (compared to 0.078 weeks in the original dataset). 

- The difference in birth weight is 0.17 pounds with (compared to 0.12 lbs in the original dataset). 

- The correlation between birth weight and mother’s age is 0.08 with (compared to 0.07). 

- The chi-squared test is statistically significant with (as it was in the original). 
...
- A way to compute p-values by generating samples, with replacement, from an observed dataset. 
...
[4<sup>]</sup> For more about Bayesian inference, see the sequel to this book, _Think Bayes_ . 

[5<sup>]</sup> Adapted from MacKay, _Information Theory, Inference, and Learning Algorithms_ , 2003. 
...
- If the residuals are uncorrelated and normally distributed with mean 0 and constant (but unknown) variance, then the least squares fit is also the maximum likelihood estimator of `inter` and `slope` . See Wikipedia. The values of `inter` and `slope` that minimize the squared residuals can be computed efficiently. 
...
`LeastSquares` takes sequences `xs` and `ys` and returns the estimated parameters `inter` and `slope` . For details on how it works, see Wikipedia. `thinkstats2` also provides `FitLine` , which takes `inter` and `slope` and returns the fitted line for a sequence of `xs` . 
...
Instead of presenting the intercept at , it is often helpful to present the intercept at the mean of x. In this case the mean age is about 25 years and the mean baby weight for a 25-year-old mother is 7.3 pounds. The slope is 0.27 ounces per year, or 0.17 pounds per decade. 
...
_Figure 10-1. Scatter plot of birth weight and mother’s age with a linear fit_ 
...
To visualize the residuals, I group respondents by age and compute percentiles in each group, as we saw in Characterizing Relationships. Figure 10-2 shows the 25th, 50th and 75th percentiles of the residuals for each age group. The median is near zero, as expected, and the interquartile range is about 2 pounds. So if we know the mother’s age, we can guess the baby’s weight within a pound, about 50% of the time. 



_Figure 10-2. Residuals of the linear fit_ 
...
The  parameters `slope` and `inter` are estimates based on a sample; like other estimates, they are vulnerable to sampling bias, measurement error, and sampling error. As discussed in Chapter 8, sampling bias is caused by nonrepresentative sampling, measurement error is caused by errors in collecting and recording data, and sampling error is the result of measuring a sample rather than the entire population. 
...
_Figure 10-3. 50% and 90% confidence intervals showing variability in the fitted line due to sampling error of_ _`inter` and_ _`slope`_ 
...
Another way to measure goodness of fit is the **coefficient of determination** , usually denoted R<sup>2</sup> and called “R-squared”: 
...
`Var(res)` is the MSE of your guesses using the model, `Var(ys)` is the MSE without it. So their ratio is the fraction of MSE  that remains if you use the model, and R<sup>2</sup> is the fraction of MSE the model eliminates. 

For birth weight and mother’s age, R<sup>2</sup> is 0.0047, which means that mother’s age predicts about half of 1% of variance in birth weight. 

There is a simple relationship between the coefficient of determination and Pearson’s coefficient of correlation: . For example, if ρ is 0.8 or -0.8, . 

Although ρ and R<sup>2</sup> are often used to quantify the strength of a relationship, they are not easy to interpret in terms of predictive power. In my opinion, `Std(res)` is the best representation of the quality of prediction, especially if it is presented in relation to `Std(ys)` . 
...
According to one study, there is a Pearson correlation of between total SAT scores and IQ scores, which sounds like a strong correlation. But , so SAT scores account for only 52% of variance in IQ. 
...
If you see a correlation that looks impressive, remember that R<sup>2</sup> is a better indicator of reduction in MSE, and reduction in RMSE is a better indicator of predictive  power. 
...
One option is to test whether the apparent reduction in MSE is due to chance. In that case, the test statistic is R<sup>2</sup> and the null hypothesis is that there is no relationship between the variables. We can simulate the null hypothesis by permutation, as in Testing a Correlation, when we tested the correlation between mother’s age and birth weight. In fact, because , a one-sided test of R<sup>2</sup> is equivalent to a two-sided test of ρ. We’ve already done that test, and found , so we conclude that the apparent relationship between mother’s age and birth weight is statistically significant. 
...
The p-value is less than , so although the estimated slope is small, it is unlikely to be due to chance. 
...
Figure 10-4 shows the  sampling distribution of the slope, from Estimation, and the distribution of slopes generated under the null hypothesis. The sampling distribution is centered about the estimated slope, 0.017 lbs/year, and the slopes under the null hypothesis are centered around 0; but other than that, the distributions are identical. The distributions are also symmetric, for reasons we will see in Central Limit Theorem. 



_Figure 10-4. The sampling distribution of the estimated slope and the distribution of slopes generated under the null hypothesis. The vertical lines are at 0 and the observed slope, 0.017_ 
...
The second option is easier because we normally want to compute the sampling distribution of the parameters anyway. And it is a good approximation unless the sample size is small _and_ the distribution of residuals is skewed. Even then, it is usually good enough, because p-values don’t have to be precise. 
...
As an example, if you survey 100,000 people in a country of 300 million, each respondent represents 3,000 people. If you oversample one group by a 

factor of 2, each person in the oversampled group would have a lower weight, about 1500. 
...
`pmf` maps from each row index to its normalized weight. Converting to a Cdf makes the sampling process faster. `indices` is a sequence of row indices; `sample` is a DataFrame that contains the selected rows. Since we sample with replacement, the same row might appear more than once. Now we can compare the effect of resampling with and without weights. Without weights, we generate the sampling distribution like this: 
...
||Mean birth weight<br>(lbs)|Standard<br>error|90% CI|
|---|---|---|---|
|Unweighte<br>d|7.27|0.014|(7.24,<br>7.29)|
|Weighted|7.35|0.014|(7.32,|
...
Using the data from the BRFSS, compute the linear least squares fit for log(weight) versus height. How would you best present the estimated parameters for a model like this where one of the variables is logtransformed? If you were trying to guess someone’s weight, how much would it help to know their height? 
...
A model of a dataset that minimizes the sum of squares of the residuals. residual 

The deviation of an actual value from a model. goodness of fit 

A measure of how well a model fits data. coefficient of determination 

A statistic intended to quantify goodness of fit. 

sampling weight 

- A value associated with an observation in a sample that indicates what part of the population  it represents. 
...
The p-value associated with `agepreg` is `5.7e-11` , which is less than , as expected. 

`results.rsquared` contains R<sup>2</sup> , which is . `results` also provides `f_pvalue` , which is the p-value associated with the model as a whole, similar to testing whether R<sup>2</sup> is statistically significant. 
...
The slope and the intercept are statistically significant, which means that they were unlikely to occur by chance, but the the R<sup>2</sup> value for this model is small, which means that `isfirst` doesn’t account for a substantial part of the variation in birth weight. 
...
Again, the parameters are statistically significant, but R<sup>2</sup> is low. 
...
R<sup>2</sup> for this model is a little higher, which indicates that the two variables together account for more variation in birth weight than either alone  (but not by much). 
...
||isfirst|agepreg|agepreg2|R<sup>2</sup>|
|---|---|---|---|---|
|Model<br>1|-0.125 *|–|–|0.002|
|Model<br>2|–|0.0175<br>*|–|0.004<br>7|
|Model<br>3|-0.0698<br>(0.025)|0.0154<br>*|–|0.005<br>3|
|Model<br>4|-0.0504 (0.11)|0.112 *|<sup>-0.00185</sup><br>*|0.007<br>5|



The columns in this table are the explanatory variables and the coefficient of determination, R<sup>2</sup> . Each entry is an estimated parameter and either a p- value in parentheses or an asterisk to indicate a p-value less  that 0.001. We conclude that the apparent difference in birth weight is explained, at least in part, by the difference in mother’s age. When we include mother’s age in the model, the effect of `isfirst` gets smaller, and the remaining effect might be due to chance. 
...
So far we have used regression  models for explanation; for example, in the previous section we discovered that an apparent difference in birth weight is actually due to a difference in mother’s age. But the R<sup>2</sup> values of those models is very low, which means that they have little predictive power. In this section we’ll try to do better. 

Suppose one of your co-workers is expecting a baby and there is an office pool to guess the baby’s birth weight (if you are not familiar with betting pools, see Wikipedia). 

Now suppose that you _really_ want to win the pool. What could you do to improve your chances? Well, the NSFG dataset includes 244 variables about each pregnancy and another 3087 variables about each respondent. 

Maybe some of those variables have predictive power. To find out which ones are most useful, why not try them all? 
...
The first line selects records for pregnancies longer than 30 weeks, assuming that the office pool is formed several weeks before the due date. The next line reads the respondent file. The result is a DataFrame with integer indices; in order to look up respondents efficiently, I replace `resp.index` with `resp.caseid` . 
...
For each variable we construct a model, compute R<sup>2</sup> , and append the results to a list. The models all include `agepreg` , since we already know that it has some predictive power. 

I check that each explanatory variable  has some variability; otherwise the results of the regression are unreliable. I also check the number of observations for each model. Variables that contains a large number of `nan` s are not good candidates for prediction. 
...
The next step is to  sort the results and select the variables that yield the highest values of R<sup>2</sup> . 
...
People who own their homes have heavier babies by about 0.12 lbs, even when we control for income. The parameter for mother’s age is smaller than what we saw in Multiple Regression, which suggests that some of the other variables are correlated with age, probably including `paydu` and `totincr` . 

All of these variables are  statistically significant, some with very low p- values, but R<sup>2</sup> is only 0.06, still quite small. RMSE without using the model is 1.27 lbs; with the model it drops to 1.23. So your chance of winning the pool is not substantially  improved. Sorry! 
...
Where y is the dependent variable, and x1 and x2 are explanatory variables. Then we could find the parameters that minimize the residuals. 

The problem  with this approach is that it produces predictions that are hard to interpret. Given estimated parameters and values for x1 and x2, the model might predict , but the only meaningful values of y are 0 and 1. 
...
or , and those are not valid probabilities. 
...
Suppose we have estimated the parameters β0, β1, and β2 (I’ll explain how in a minute). And suppose we are given values for x1 and x2. We can compute the predicted value of , and then convert to a probability: 
...
And we start with the initial guesses , , and : 
...
The overall likelihood of the data is the product of `likes` : `>>> like = np.prod(likes) 0.18` 
...
The coefficient of determination, R<sup>2</sup> , does not apply to logistic regression, but there are several alternatives that are used as “pseudo R<sup>2</sup> values.” These 

values can be useful for comparing models. For example, here’s a model that includes several factors believed to be associated with sex ratio: 
...
None of the estimated parameters are statistically significant. The pseudoR<sup>2</sup> value is a little higher, but that could be  due to chance. 
...
The variables in a regression model we would like to predict. Also known as endogenous variables. 

###### explanatory variables 

The variables used to predict or explain the dependent variables. Also known as independent, or exogenous, variables. 
...
- A linear regression that estimates parameters by minimizing the squared error of the residuals. 
...
- proxy variable 
   ...
   - A variable that contributes information to a regression model indirectly because of a relationship with another factor, so it acts as a proxy for that factor. 
...
- join 
   ...
   - An operation that combines data from two DataFrames using a key to match up rows in the two frames. 

###### data mining 

   - An approach to finding relationships between variables by testing a large number of models. 
...
A form of regression used when the dependent variable is a non-negative integer, usually a count. 

###### odds 

An alternative way of representing a probability, p, as the ratio of the . probability and its complement, 
...
High, medium, or low quality, as reported by the purchaser date 
...
Approximate latitude of the transaction, based on city name lon 
...
`PrePlot` with `rows=3` means that we are planning to make three subplots laid out in three rows. The loop iterates through the DataFrames and creates 

a scatter plot for each. It is common to plot time series with line segments between the points, but in this case there are many data points and prices are highly variable, so adding lines would not help. 

Since the labels on the x-axis are dates, I use `pyplot.xticks` to rotate the “ticks” 30 degrees, making them more readable. 

Figure 12-1 shows the result. One apparent feature in these plots is a gap around November 2013. It’s possible that data collection was not active during this time, or the data might not be available. We will consider ways to deal with this missing data later. 
...
|Quality|<sup>Intercep</sup><br>t|Slope|R<sup>2</sup>|
|---|---|---|---|
|High|13.450|-0.70<br>8|0.44<br>4|
|Mediu<br>m|8.879|0.283|0.05<br>0|
...
_Figure 12-1. Time series of daily price per gram for high, medium, and low quality cannabis_ 

The R<sup>2</sup> value for high quality cannabis is 0.44, which means that time as an explanatory variable accounts for 44% of the observed variability in price. For the other qualities, the change in price is smaller, and variability in prices is higher, so the values of R<sup>2</sup> are smaller (but still statistically significant). 
...
As we saw in Implementation, `model` contains `exog` and `endog` , NumPy arrays with the exogenous (explanatory) and endogenous (dependent) variables. 

`PlotFittedValues` makes a scatter plot  of the data points and a line plot of the fitted values. Figure 12-2 shows the results for high quality cannabis. 



_Figure 12-2. Time series of daily price per gram for high quality cannabis, and a linear least squares fit_ 
...
A smooth function that captures persistent changes Seasonality 
...
The first two values are `nan` ; the next value is the mean of the first three elements, 0, 1, and 2. The next value is the mean of 1, 2, and 3. And so on. Before we can apply `rolling_mean` to the cannabis data, we have to deal with missing values. There are a few days in the observed interval with no reported transactions for one or more quality categories, and a period in 2013 when data collection was not active. 

In the DataFrames we have used so far, these dates are absent; the index skips days with no data. For the analysis that follows, we need to represent this missing data explicitly. We can do that by “reindexing” the DataFrame: `dates = pandas.date_range(daily.index.min(), daily.index.max()) reindexed = daily.reindex(dates)` 
...
Figure 12-3 (left) shows the result. The rolling mean seems to do a good job of smoothing out the noise and extracting the trend. The first 29 values are `nan` , and wherever there’s a missing value, it’s followed by another 29 `nan` s. There are ways to fill in these gaps, but they are a minor nuisance. 



_Figure 12-3. Daily price and a rolling mean (left) and exponentially-weighted moving average (right)_ 

An alternative is the **exponentially-weighted moving average** (EWMA), which has two advantages. First, as the name suggests, it computes a weighted average where the most recent value has the highest weight and 

the weights for previous values drop off exponentially. Second, the pandas implementation of EWMA handles missing values better. 
...
`resid` contains the residual values, not including days when `ppg` is `nan` . `fake_data` contains the sum of the moving average and a random sample of residuals. Finally, `fillna` replaces `nan` with values from `fake_data` . Figure 12-4 shows the result. The filled data is visually similar to the actual values. Since the resampled residuals are random, the results are different every time; later we’ll see how to characterize the error created by missing values. 



_Figure 12-4. Daily price with filled data_ 
...
|La<br>g|High|<sup>Mediu</sup><br>m|Low|
|---|---|---|---|
|1|-0.02<br>9|-0.014|0.034|
|7|0.02|-0.042|-0.009<br>7|
|30|0.014|-0.0064|-0.013|
|36<br>5|0.045|0.015|0.033|
...
StatsModels, which we used for linear regression in StatsModels, also provides functions for time series analysis,  including `acf` , which computes the autocorrelation function: 
...
`frisat` is a boolean Series, `True` if the day of the week is Friday or Saturday. `fake` is a new DataFrame, initially a copy of `daily` , which we modify by adding random values to `ppg` . `frisat.sum()` is the total number of Fridays and Saturdays, which is the number of random values we have to generate. 

Figure 12-5 (right) shows autocorrelation functions for prices with this simulated seasonality. As expected, the correlations are highest when the lag is a multiple of 7. For high and medium quality, the new correlations are statistically significant. For low quality they are not, because residuals  in 

this category are large; the effect would have to be bigger to be visible through the noise. 
...
The linear regressions we used in Linear Regression can be used for prediction. The RegressionResults class provides `predict` , which takes a DataFrame containing the explanatory variables and returns a sequence of predictions. Here’s the code: 
...
The prediction is based on estimated parameters, which depend on random variation in the sample. If we run the experiment again, we expect the estimates to vary. 
...
Another source of error to consider is unexpected future events. Agricultural prices are affected by weather, and all prices are affected by 

politics and law. As I write this, cannabis is legal in two states and legal for medical purposes in 20 more. If more states legalize it, the price is likely to go down. But if the federal government cracks down, the price might go up. Modeling errors and unexpected future events are hard to quantify. Sampling error and random variation are easier to deal with, so we’ll do that first. 

To quantify sampling error, I use resampling, as we did in Estimation. As always, the goal is to use the actual observations to simulate what would happen if we ran the experiment again. The simulations are based on the assumption that the estimated parameters are correct, but the random residuals could have been different. Here is a function that runs the simulations: 
...
`SimulateResults` uses `RunLinearModel` , from Linear Regression, to estimate the slope and intercept of the observed values. 
...
_Figure 12-6. Predictions based on linear fits, showing variation due to sampling error and prediction error_ 

The lighter region shows a 90% confidence interval for prediction error, which is the sum of sampling error and random variation. These regions quantify sampling error and random variation, but not modeling error. In general modeling error is hard to quantify, but in this case we can address at least one source of error, unpredictable external events. 
...
But looking at the moving averages in Figure 12-3, it seems like the slope changes at least once during the observed interval, and the variance of the 

residuals seems bigger in the first half than the  second. 
...
_Figure 12-7. Predictions based on linear fits, showing variation due to the interval of observation_ 

The model based on the entire interval has positive slope, indicating that prices were increasing. But the most recent interval shows signs of decreasing prices, so models based on the most recent data have negative 

slope. As a result, the widest predictive interval includes the possibility of decreasing prices over the  next year. 
...
But once you have learned the material in this chapter, you are well prepared to learn about autoregression. One resource I recommend is Philipp Janert’s book, _Data Analysis with Open Source Tools_ , O’Reilly Media, 2011. His chapter on time series analysis picks up where this one leaves off. 
...
The linear model I used in this chapter has the obvious drawback that it is linear, and there is no reason to expect prices to change linearly over time. We can add flexibility to the model by adding a quadratic term, as we did in Nonlinear Relationships. 
...
Use this class to test whether the serial correlation in raw price data is statistically significant. Also test the residuals of the linear model and (if 
...
- Use `Series.diff` to compute differences between successive elements. Use `reindex` to extend the DataFrame index into the future. Use `fillna` to put your predicted values into the DataFrame. 

## **Glossary** 

###### time series 

A dataset where each value is associated with a timestamp, often a series of measurements and the times they were collected. 

###### window 

- A sequence of consecutive values in a time series, often used to compute a moving average. 

###### moving average 

One of several statistics intended to estimate the underlying trend in a time series by computing averages (of some kind) for a series of 
...
- exponentially-weighted moving average (EWMA) 
   ...
   - A moving average based on a weighted mean that gives the highest weight to the most recent values, and exponentially decreasing weights to earlier values. 
...
A more general term for a serial correlation with any amount of lag. autocorrelation function 

A function that maps from lag to serial correlation. 

###### stationary 

A model is stationary if the parameters and the distribution of residuals does not change over time. 
...
The code in this chapter is in `survival.py` . For information about downloading and working with this code, see Using the Code. 

## **Survival Curves** 

The fundamental concept in  survival analysis is the **survival curve** , , which is a function that maps from a duration, t, to the probability of surviving longer than t. If you know the distribution of durations, or “lifetimes”, finding the survival curve is easy; it’s just the complement of the CDF: 



where is the probability of a lifetime less than or equal to t. For example, in the NSFG dataset, we know the duration of 11189 complete pregnancies. We can read this data and compute the CDF: 
...
_Figure 13-1. Cdf and survival function for pregnancy length (top), hazard function (bottom)_ 

`SurvivalFunction` provides two properties: `ts` , which is the sequence of lifetimes, and `ss` , which is the survival function. In Python, a “property” is 

a method that can  be invoked as if it were a variable. 

We can instantiate a `SurvivalFunction` by passing the CDF of lifetimes: `sf = SurvivalFunction(cdf)` 
...
`SurvivalFunction` provides `Render` , so we can plot `sf` using the functions in `thinkplot` : `thinkplot.Plot(sf)` 
...
From the survival function  we can derive the **hazard function** ; for pregnancy lengths, the hazard function maps from a time, t, to the fraction of pregnancies that continue until t and then end at t. To be more precise: 
...
The `HazardFunction` object is a wrapper for a pandas Series: `class HazardFunction(object):` 
...
`HazardFunction` provides `__getitem__` , so we can evaluate it like this: `>>> hf = sf.MakeHazard() >>> hf[39] 0.49689` 

So of all pregnancies that proceed until week 39, about 50% end in week 39. 

Figure 13-1 (bottom) shows the hazard function for pregnancy lengths. For times after week 42, the hazard function is erratic because it is based on a small number of cases. Other than that the shape of the curve is as expected: it is highest around 39 weeks, and a little higher in the first trimester than in the second. 
...
For example, suppose you are following a group of patients to see how long they survive after diagnosis. Not all patients are diagnosed on the same day, 

so at any point in time, some patients have survived longer than others. If some patients have died, we know their survival times. For patients who are still alive, we don’t know survival times, but we have a lower bound. 
...
First, we precompute `hist_complete` , which is the Hist of ages when women were married, `sf_complete` , the survival function for married women, and `sf_ongoing` , the survival function for unmarried women. The loop iterates through the ages when respondents got married. For each value of `t` , we have `ended` , which is the number of women who got married at age `t` . Then we compute the number of women “at risk”, which is the sum of: 

- `ended` , the number of respondents married at age `t` , `n * sf_complete[t]` , the number of respondents married after age `t` . `m * sf_ongoing[t]` , the number of unmarried respondents interviewed after `t` , and therefore known not to have been married at or before `t` . 

The estimated value of the hazard function at `t` is the ratio of `ended` to `at_risk` . 

`lams` is a dictionary that maps from t to . The result is a `HazardFunction` object. 
...
The respondent’s date of birth, known for all respondents. `cmintvw` 

The date the respondent was interviewed, known for all respondents. `cmmarrhx` 

The date the respondent was first married, if applicable and known. `evrmarry` 

This is 1 if the respondent had been married prior to the date of interview; otherwise, 0. 
...
`ts` is the sequence of times where the hazard function is estimated. `ss` is the cumulative product of the complementary hazard function, so it is the survival function. 
...
Figure 13-2 (bottom) shows the result. The survival curve is steepest between 25 and 35, when most women get married. Between 35 and 45, the curve is nearly flat, indicating that women who do not marry before age 35 are unlikely to get married. 



_Figure 13-2. Hazard function for age at first marriage (top) and survival function (bottom)_ 

A curve like this was the basis of a famous magazine article in 1986; _Newsweek_ reported that a 40-year old unmarried woman was “more likely to be killed by a terrorist” than get married. These statistics were widely reported and became part of popular culture, but they were wrong then (because they were based on faulty analysis) and turned out to be even more wrong (because of cultural changes that were already in progress and continued). In 2006, _Newsweek_ ran an another article admitting that they were wrong. 
...
`ResampleSurvival` takes `resp` , a DataFrame of  respondents, and `iters` , the number of times to resample. It computes `ts` , which is the sequence of ages where we will evaluate the survival functions. 

Inside the loop, `ResampleSurvival` : 

- Resamples the respondents using `ResampleRowsWeighted` , which we saw in Weighted Resampling. 

- Calls `EstimateSurvival` , which uses the process in the previous sections to estimate the hazard and survival curves, and Evaluates the survival curve at each age in `ts` . 

`ss_seq` is a sequence of evaluated survival curves. `PercentileRows` takes this sequence and computes the 5th and 95th percentiles, returning a 90% confidence interval for the survival curve. 

Figure 13-3 shows the result along with the survival function we estimated in the previous section. The confidence interval takes into account the sampling weights, unlike the estimated curve. The discrepancy between them indicates that the sampling weights have a substantial effect on the estimate — we will have to keep that in  mind. 



_Figure 13-3. Survival function for age at first marriage and a 90% confidence interval based on weighted resampling._ 
...
To investigate cohort effects in the NSFG marriage data, I gathered the Cycle 6 data from 2002 used throughout this book; the Cycle 7 data from 2006–2010 used in Replication; and the Cycle 5 data from 1995. In total these datasets include 30,769 respondents. 
...
For each DataFrame, `resp` , I use `cmbirth` to compute the decade of birth for each respondent: 
...
`cmbirth` is encoded as the integer number of months since December 1899; `month0` represents that date as a Timestamp object. For each birth date, we instantiate a `DateOffset` that contains the century-month and add it to `month0` ; the result is a sequence of Timestamps, which is converted to a `DateTimeIndex` . Finally, we extract `year` and compute decades. 
...
Data from the three NSFG cycles use different sampling weights, so I resample them separately and then use `concat` to merge them into a single DataFrame. The parameter `ignore_index` tells `concat` not to match up respondents by index; instead it creates a new index from 0 to 30768. 
...
_Figure 13-4. Survival functions for respondents born during different decades._ 

Several patterns are visible: 

- Women born in the 1950s married earliest, with successive cohorts marrying later and later, at least until age 30 or so. 

- Women born in the 1960s follow a surprising pattern. Prior to age 25, they were marrying at slower rates than their predecessors. After age 25, they were marrying faster. By age 32, they had overtaken the 50s cohort, and at age 44 they are substantially more likely to have married. Women born in the 60s turned 25 between 1985 and 1995. Remembering that the _Newsweek_ article I mentioned was published in 1986, it is tempting to imagine that the article triggered a marriage boom. That explanation would be too pat, but it is possible that the article and the reaction to it were indicative of a mood that affected the behavior of this cohort. 

- The pattern of the 1970s cohort is similar. They are less likely than their predecessors to be married before age 25, but at age 35 they have caught up with both of the previous cohorts. Women born in the 1980s are even less likely to marry before age 25. What happens after that is not clear; for more data, we have to wait for the next cycle of the NSFG. 
...
We can extrapolate these curves by “borrowing” data from the previous cohort. HazardFunction provides a method, `Extend` , that copies the tail from another longer HazardFunction: 
...
As we saw in Hazard Function, the HazardFunction contains a Series that maps from t to . `Extend` finds `last` , which is the last index in `self.series` , selects values from `other` that come later than `last` , and appends them onto `self.series` . 
...
Figure 13-5 shows the results; I’ve removed the 50s cohort to make the predictions more visible. These results suggest that by age 40, the most recent cohorts will converge with the 60s cohort, with fewer than 20% never married. 



_Figure 13-5. Survival functions for respondents born during different decades, with predictions for the later cohorts._ 

## **Expected Remaining Lifetime** 

Given a survival curve, we can compute the expected remaining lifetime as a function of current age. For example, given the survival function of 

pregnancy length from Survival Curves, we can compute the expected time until delivery. 
...
_Figure 13-6. Expected remaining lifetime for pregnancy length (left) and years until first marriage (right)_ 
...
Between Week 37 and 42, the curve levels off between 1 and 2 weeks. At any time during this period, the expected remaining lifetime is the same; with each week that passes, the destination gets no closer. Processes with this property are called _memoryless_ because the past has no effect on the 

predictions. This behavior is the mathematical basis of the infuriating mantra of obstetrics nurses: “any day now.” 
...
A function that maps from a time, t, to the probability of surviving past t. hazard function 
...
To answer that question, we need the sampling distribution of . In Sampling Distributions we approximated this distribution by simulating the experiment (weighing 9 gorillas), computing for each simulated experiment, and accumulating the distribution of estimates. 
...
- The interval between the 5th and 95th percentile of the sampling distribution  is a 90% confidence interval. If we run the experiment many times, we expect the estimate to fall in this interval 90% of the time. In the example, the 90% CI is kg. 

Now we’ll do the same calculation analytically. We take advantage of the fact that the weights of adult female gorillas are roughly normally distributed. Normal distributions have two properties that make them 

amenable for analysis: they are “closed” under linear transformation and addition. To explain what that means, I need some notation. If the distribution of a quantity, X, is normal with parameters μ and σ, you can write 



where the symbol ∼ means “is distributed” and the script letter stands for “normal.” 

A linear transformation of X is something like , where a and b are real numbers. A family of distributions is closed under linear transformation if is in the same family as X. The normal distribution has this property; if , 



Normal distributions are also closed under addition. If and and then 
...
Now we have everything we need to compute the sampling distribution of . Remember that we compute by weighing n gorillas, adding up the total 

weight, and dividing by n. Assume that the distribution of gorilla  weights, X, is approximately normal: 
...
The distribution of Z is the sampling distribution of . The mean of Z is μ, which shows that is an unbiased estimate of μ. The variance of the . sampling distribution is 

So the standard deviation of the sampling distribution, which is the standard error of the estimate, is . In the example, σ is 7.5 kg and n is 9, so the standard error is 2.5 kg. That result is consistent with what we estimated by simulation, but much faster to compute! 

We can also use the sampling distribution to compute confidence intervals. A 90% confidence interval for is the interval between the 5th and 95th percentiles of Z. Since Z is normally distributed, we can compute percentiles by evaluating the inverse CDF. 

There is no closed form for the CDF of the normal distribution or its inverse, but there are fast numerical methods and they are implemented in SciPy, as we saw in The Normal Distribution. `thinkstats2` provides a wrapper function that makes the SciPy function a little easier to use: 
...
Given a probability, `p` , it returns the corresponding percentile from a normal distribution with parameters `mu` and `sigma` . For the 90% confidence interval of , we compute the 5th and 95th percentiles like this: 
...
So if we run the experiment many times, we expect the estimate, , to fall in the range about 90% of the time. Again, this is consistent with the result we got by simulation. 
...
- The values have to be drawn independently. If they are correlated, the CLT doesn’t apply (although this is seldom a problem in practice). The values have to come from the same distribution (although this requirement can be relaxed). 
...
_Figure 14-1. Distributions of sums of exponential values (top row) and lognormal values (bottom row)_ 

Pareto distributions are even more skewed than lognormal. Depending on the parameters, many Pareto distributions do not have finite mean and variance. As a result, the Central Limit Theorem does not apply. Figure 142 (top row) shows distributions of sums of Pareto values. Even with `n=100` the normal probability plot is far from  straight. 



_Figure 14-2. Distributions of sums of Pareto values (top row) and correlated exponential values (bottom row)_ 
...
To see why the Central Limit Theorem is useful, let’s get back to the example in Testing a Difference in Means: testing the apparent difference in mean pregnancy length for first babies and others. As we’ve seen, the apparent difference is about 0.078 weeks: 
...
Both sampling distributions are based on the same population, which is the pool of all live births. `SamplingDistMean` takes this sequence of values and 
...
So we can compute the sampling distribution of the difference like this: `>>> dist = dist1 - dist2 N(0, 0.0032)` 
...
Which is the same because the normal distribution is symmetric. The sum of the tails is 0.168, which is consistent with the estimate in Testing a Difference in Means, which was 0.17. 

## **Correlation Test** 

In Testing a Correlation we used a permutation test for the correlation between birth weight and mother’s age, and found that it is statistically significant, with p-value less than 0.001. 
...
the distribution of t is Student’s t-distribution with parameter . The t- distribution is an analytic distribution; the CDF can be computed efficiently using gamma functions. 
...
We compute the value of `t` that corresponds to `r=0.07` , and then evaluate the t-distribution at `t` . The result is `6.4e-12` . This example demonstrates an 

advantage of the analytic method: we can compute very small p-values. But in practice it usually doesn’t  matter. 

## **Chi-Squared Test** 

In Chi-Squared Tests we used the  chi-squared statistic to test whether a die is crooked. The chi-squared statistic measures the total normalized deviation from the expected values in a table: 



One reason the chi-squared statistic is widely used is that its sampling distribution under the null hypothesis is analytic; by a remarkable coincidence,<sup>[</sup> <u>6</u><sup>]</sup> it is called the chi-squared distribution. Like the t- distribution, the chi-squared CDF can be computed efficiently using gamma functions. 
...
_Figure 14-4. Sampling distribution of chi-squared statistics for a fair six-sided die._ 
...
The result is 0.041, which is consistent with the result from Chi-Squared Tests. 
...
1. Use computational methods during exploration. If you find a satisfactory answer and the runtime is acceptable, you can stop. 
...
In The lognormal Distribution, we saw that the distribution of adult weights is approximately lognormal. One possible explanation is that the weight a 

person gains each year is proportional to their current weight. In that case, adult weight is the product of a large number of multiplicative factors: 
...
So by the Central Limit Theorem, the distribution of is approximately normal for large n, which implies that the distribution of w is lognormal. To model this phenomenon, choose a distribution for f that seems reasonable, then generate a sample of adult weights by choosing a random value from the distribution of birth weights, choosing a sequence of factors from the distribution of f, and computing the product. What value of n is needed to converge to a lognormal distribution? 

_Example 14-2._ 

In Applying the CLT we used the Central Limit Theorem to find the sampling distribution of the difference in means, δ, under the null hypothesis that both samples are drawn from the same population. 
...
In a recent paper<sup>[</sup> <u>7</u><sup>]</sup> , Stein et al. investigate the effects of an intervention intended to mitigate gender-stereotypical task allocation within student engineering teams. 
...
[6<sup>]</sup> Not really. 

[7<sup>]</sup> “Evidence for the persistent effects of an intervention to mitigate gender-sterotypical task allocation within student engineering teams,” Proceedings of the IEEE Frontiers in Education Conference, 2014. 

## **Index** 

#### **A** 

abstractions, <u>Why Model?</u> accuracy (models), <u>Accuracy</u> acf function, <u>Autocorrelation</u> Adams, Cecil, <u>Exercises</u> addition, closed under, <u>Normal Distributions</u> adult weight, <u>The lognormal Distribution–The lognormal Distribution, Relationships Between Variables–Scatter Plots, Sampling Bias, Exercises</u> age, <u>Implementation, StatsModels–Multiple Regression, Nonlinear Relationships, Implementation, Estimating Survival Curves, The Marriage Curve</u> age groups, <u>Comparing Percentile Ranks</u> aggregate method, <u>Importing and Cleaning</u> alpha parameter, <u>Scatter Plots</u> Anaconda distribution, <u>Using the Code, StatsModels</u> analytic distributions, <u>Modeling Distributions, Glossary, Correlation Test</u> analytic methods, <u>Analytic Methods</u> anecdotal evidence, <u>Exploratory Data Analysis, Glossary</u> apparent effect, <u>Classical Hypothesis Testing</u> artifacts, <u>Scatter Plots</u> Australia, baby birth times, <u>The Exponential Distribution</u> autocorrelation function, <u>Autocorrelation–Autocorrelation, Glossary</u> average, <u>Summarizing Distributions</u> **B** 

bar plots, <u>Plotting PMFs</u> 

Bayesian inference, <u>Classical Hypothesis Testing</u> Bayesian statistics, <u>Sampling Distributions</u> 

Behavioral Risk Factor Surveillance System (BRFSS), <u>Preface, The lognormal Distribution, Scatter Plots, Spearman’s Rank Correlation, Exercises</u> 

betting pool, <u>Data Mining, Logistic Regression, Exercises</u> 

bias 

confirmation, <u>Exploratory Data Analysis</u> observer, <u>The Class Size Paradox, Exercises–Exercises</u> sampling, <u>Sampling Bias, Glossary, Estimation</u> selection, <u>Exploratory Data Analysis, Exercises</u> skewness and, Skewness 

biased coin, <u>HypothesisTest</u> 

biased estimators, <u>Guess the Variance–Sampling Distributions, Exponential Distributions–Glossary, Sampling Distributions</u> 

binary integers, <u>Implementation</u> 

binary searches, <u>Cdf Implementation–Cdf Implementation</u> 

binning data, <u>The Limits of PMFs, The Distribution Framework, Characterizing Relationships</u> 

birth times, <u>The Exponential Distribution</u> 

birth weight 

about, <u>Exercises</u> comparing CDFs for, <u>Comparing CDFs</u> computing distribution of, <u>Exercises</u> correlation test and, <u>Correlation Test</u> 

hypothesis testing, <u>Testing a Difference in Means–Testing a Correlation, Replication–Exercises</u> 

least squares fit, <u>Implementation–Implementation</u> 

limitation of PMFs, <u>The Limits of PMFs</u> 

normal distribution, <u>The Normal Distribution</u> 

normal probability plot, <u>Normal Probability Plot–The lognormal Distribution</u> 

predictions with linear model, <u>Goodness of Fit–Testing a Linear Model</u> random samples, <u>Random Numbers</u> 

regression analysis, <u>Regression, Multiple Regression–Data Mining, Prediction–Prediction</u> 

variables for, <u>Variables, NSFG Variables</u> 

weighted resampling, <u>Weighted Resampling</u> 

bisect module, <u>Cdf Implementation</u> 

Blue Man Group, <u>Exercises</u> 

Boolean type, <u>Validation, NSFG Variables, Scatter Plots, Multiple Regression, Prediction–Logistic Regression, Implementation, Autocorrelation, Survival Curves</u> 

bracket operator, <u>Representing Histograms–NSFG Variables, Pmfs, Representing CDFs</u> 

BRFSS (Behavioral Risk Factor Surveillance System), <u>Preface, The lognormal Distribution, Scatter Plots, Spearman’s Rank Correlation, Exercises</u> 

Bureau of Labor Statistics, <u>Exercises</u> 

**C** 

cannabis, <u>Time Series Analysis</u> 

casinos, <u>Testing Proportions</u> 

categorical variables, <u>Multiple Regression, Prediction–Logistic Regression, Implementation, Exercises–Glossary</u> 

causal relationships, <u>Correlation and Causation</u> 

causation, <u>Correlation and Causation</u> 

CCDF (complementary CDF), <u>The Exponential Distribution, The Pareto Distribution, Glossary, Survival Curves, Estimating the Survival Function</u> 

Cdf class 

about, <u>Representing CDFs</u> 

computing percentile-based statistics, <u>Percentile-Based Statistics–Random Numbers</u> 

distribution framework, <u>The Distribution Framework</u> 

implementing, <u>Cdf Implementation–Cdf Implementation</u> 

random numbers and, <u>Random Numbers</u> 

survival function and, <u>Survival Curves, Expected Remaining Lifetime</u> weighted resampling, <u>Weighted Resampling</u> 

CDFs (cumulative distribution functions) 

about, <u>CDFs–Representing CDFs, Glossary</u> 

characterizing relationships, <u>Characterizing Relationships</u> comparing, <u>Comparing CDFs</u> 

comparing percentile ranks, <u>Comparing Percentile Ranks</u> 

complementary, <u>The Exponential Distribution, The Pareto Distribution, Glossary, Survival Curves, Estimating the Survival Function</u> 

derivatives of, <u>PDFs</u> 

difference in pregnancy length, <u>Testing a Difference in Means</u> distribution framework, <u>The Distribution Framework</u> 

exponential distributions, <u>The Exponential Distribution–The Exponential Distribution</u> 

inverse, <u>Representing CDFs, Random Numbers, Glossary, Generating Random Numbers, Cdf Implementation, Sampling Distributions, Testing the CLT–Testing the CLT</u> 

limits of PMFs, <u>The Limits of PMFs</u> 

lognormal distribution, <u>The lognormal Distribution</u> 

normal distribution, <u>The Normal Distribution</u> 

Pareto distribution, <u>The Pareto Distribution–The Pareto Distribution</u> percentile-based statistics, <u>Percentile-Based Statistics</u> 

percentiles, <u>Percentiles</u> 

random numbers, <u>Random Numbers</u> representing, <u>Representing CDFs</u> 

simulating null hypothesis, <u>Errors</u> 

Central Limit Theorem (CLT), <u>Preface, Central Limit Theorem–Correlation Test, Exercises</u> 

central moments, <u>Moments, Glossary</u> central tendency, <u>Summarizing Distributions, Glossary, Percentile-Based Statistics, Skewness</u> 

century-months, <u>The Marriage Curve, Cohort Effects</u> chi-squared distribution, <u>Chi-Squared Test</u> 

chi-squared statistic, <u>Chi-Squared Tests, First Babies Again</u> chi-squared test, <u>Chi-Squared Tests, Glossary, Chi-Squared Test</u> city size, <u>The Pareto Distribution</u> 

class size paradox, <u>The Class Size Paradox–The Class Size Paradox</u> cleaning data, <u>Preface, Transformation, Glossary, Data Mining, Importing and Cleaning–Importing and Cleaning</u> 

clinically significant effect, <u>Reporting Results, Glossary</u> cloning repositories, <u>Using the Code</u> closed form, <u>The Normal Distribution, Estimating Parameters, Sampling Distributions</u> 

CLT (Central Limit Theorem), <u>Preface, Central Limit Theorem–Correlation Test, Exercises</u> 

coefficient of determination, <u>Goodness of Fit–Testing a Linear Model, Glossary, StatsModels, Multiple Regression–Prediction, Implementation</u> cohort, <u>Glossary</u> 

cohort effects, <u>Cohort Effects–Cohort Effects, Glossary</u> cohorts, <u>Cohort Effects</u> 

complementary CDF, <u>The Exponential Distribution, The Pareto Distribution, Glossary, Survival Curves, Estimating the Survival Function</u> compression (data), <u>Why Model?</u> 

computational methods, <u>Preface, Least Squares Fit, Analytic Methods, Discussion</u> 

confidence interval, <u>Sampling Distributions, Sampling Bias–Glossary, Estimation, Weighted Resampling, Prediction–Prediction, Confidence Intervals–Confidence Intervals, Normal Distributions, Sampling Distributions, Exercises</u> 

confirmation bias, <u>Exploratory Data Analysis</u> confirmatory results, <u>Replication</u> 

continuous distribution, <u>The Distribution Framework</u> contradiction, proof by, <u>Classical Hypothesis Testing</u> 

contributor list, <u>Contributor List</u> 

control groups, <u>Correlation and Causation–Glossary</u> control variables, <u>Preface, Nonlinear Relationships, Prediction, Glossary</u> 

controlled trials, <u>Correlation and Causation–Glossary</u> 

correct positive rate, <u>Power</u> correlation about, <u>Correlation, Glossary</u> causation and, <u>Correlation and Causation</u> computing, <u>Preface</u> Pearson’s, <u>Correlation, Pearson’s Correlation–Spearman’s Rank Correlation, Exercises, Testing a Correlation</u> residuals and, <u>Least Squares Fit</u> serial, <u>Serial Correlation–Autocorrelation, Glossary</u> Spearman’s rank correlation, <u>Correlation, Spearman’s Rank Correlation– Exercises, Testing a Correlation</u> testing, <u>Testing a Correlation</u> cost functions, <u>Least Squares Fit</u> covariance, <u>Covariance, Glossary</u> crooked die, <u>Testing Proportions</u> cross-sectional studies, <u>The National Survey of Family Growth, Glossary</u> cumulative distribution functions (see CDFs) cumulative probability, <u>Glossary, The Distribution Framework, Cdf Implementation</u> cumulative product, <u>Estimating the Survival Function</u> Current Population Survey, Exercises cycles, <u>The National Survey of Family Growth</u> **D** 

data cleaning, <u>Preface, Transformation, Glossary, Data Mining, Importing and Cleaning–Importing and Cleaning</u> 

data collection, <u>A Statistical Approach</u> data mining, <u>Data Mining–Prediction, Exercises, Glossary</u> DataFrame data structure about, <u>DataFrames–DataFrames</u> autocorrelation functions and, <u>Autocorrelation</u> cohort effects and, <u>Cohort Effects</u> comparing values, <u>First Babies</u> counting values, <u>Validation</u> difference in pregnancy length, <u>Testing a Difference in Means</u> estimating statistics, <u>Exercises</u> exponential distribution and, <u>The Exponential Distribution</u> extracting height and weight, <u>Scatter Plots</u> groupby method, <u>Characterizing Relationships</u> importing and cleaning data, <u>Importing and Cleaning–Importing and Cleaning</u> index column, <u>DataFrames, DataFrame Indexing–DataFrame Indexing, Glossary</u> interpreting values, <u>Interpretation–Exercises</u> join operation and, <u>Data Mining, Glossary</u> linear regression and, <u>Linear Regression–Linear Regression</u> logistic regression and, <u>Implementation, Accuracy</u> NSFG variables, <u>NSFG Variables</u> OLS object and, <u>StatsModels</u> plotting for time series, <u>Plotting</u> query method, <u>Survival Curves</u> 

reindexing, <u>Moving Averages</u> resampling and, <u>Estimation</u> sample size and, <u>Exercises</u> survival function and, <u>Confidence Intervals</u> weighted resampling, <u>Weighted Resampling</u> date of birth, <u>Exercises, The Marriage Curve</u> datetime64 object, <u>Importing and Cleaning–Importing and Cleaning</u> DateTimeIndex, <u>Cohort Effects</u> debugging, <u>Preface, Discussion</u> deciles, <u>Percentile-Based Statistics</u> 

degrees of freedom, <u>Guess the Variance, Correlation Test, Chi-Squared Test</u> density, <u>PDFs, Kernel Density Estimation</u> 

dependent variables, <u>Regression–StatsModels, Nonlinear Relationships, Logistic Regression–Logistic Regression, Implementation, Glossary, Linear Regression</u> 

derivatives, <u>PDFs</u> descriptive statistics, <u>A Statistical Approach</u> dice, <u>The Estimation Game, Testing Proportions</u> dictionaries, <u>Distributions, Hist Implementation</u> DictWrapper class, <u>Hist Implementation–Cdf Implementation</u> digitize function, <u>Characterizing Relationships</u> discrete distribution, <u>The Distribution Framework</u> discretizing, <u>The Distribution Framework, Glossary</u> distribution framework, <u>The Distribution Framework</u> distributions 

about, <u>Distributions, Glossary</u> analytic, <u>Modeling Distributions, Glossary, Correlation Test</u> chi-squared, <u>Chi-Squared Test</u> comparing, <u>Comparing CDFs</u> continuous, <u>The Distribution Framework</u> discrete, <u>The Distribution Framework</u> effect size in, <u>Effect Size</u> empirical, <u>Modeling Distributions, Exercises–Glossary</u> estimation game, <u>The Estimation Game–The Estimation Game</u> exploratory process, <u>Preface</u> exponential, <u>The Exponential Distribution–The Exponential Distribution, Generating Random Numbers, Exercises, PDFs, The Distribution Framework, Exponential Distributions–Exercises, Central Limit Theorem</u> first babies, <u>First Babies–First Babies</u> 

lognormal, <u>The lognormal Distribution–The lognormal Distribution, The Pareto Distribution, Exercises, Spearman’s Rank Correlation, Central Limit Theorem–Testing the CLT, Exercises</u> 

normal, <u>NSFG Variables, Glossary, The Normal Distribution–The lognormal Distribution, Exercises, PDFs, The Distribution Framework, Correlation, Spearman’s Rank Correlation, The Estimation Game, Guess the Variance, Normal Distributions</u> 

NSFG variables, <u>NSFG Variables</u> 

outliers in, <u>Outliers–Outliers</u> 

Pareto, <u>The Pareto Distribution–The Pareto Distribution, Why Model?– Exercises, Central Limit Theorem–Testing the CLT</u> plotting histograms, <u>Plotting Histograms</u> reporting results, <u>Reporting Results</u> 

representing histograms, <u>Representing Histograms</u> 

sampling, <u>Sampling Distributions–Sampling Distributions, Exercises– Glossary, Estimation, Estimation, Testing a Linear Model–Weighted Resampling, Normal Distributions–Representing Normal Distributions, Applying the CLT–Correlation Test</u> 

Student’s t, <u>Correlation Test</u> 

summarizing, <u>Summarizing Distributions</u> 

symmetric, <u>Skewness, Testing a Linear Model, Applying the CLT</u> uniform, <u>NSFG Variables, Glossary, Exercises, Generating Random Numbers, Scatter Plots, Correlation, Autocorrelation</u> 

variance in, <u>Variance</u> 

Weibull, <u>Exercises</u> 

dot product, <u>Covariance</u> 

dropna function, <u>Skewness–Skewness, Characterizing Relationships, Testing a Correlation, Implementation, Testing a Linear Model, Missing Values</u> 

**E** 

effect size, <u>Effect Size, Glossary, Testing a Difference in Means, Power, Glossary</u> empirical distributions, <u>Modeling Distributions, Exercises–Glossary</u> endogenous variables (see dependent variables) 

error (see specific type of error) 

EstimatedPdf class, <u>Kernel Density Estimation</u> 

estimation 

about, <u>A Statistical Approach, The Estimation Game, Glossary</u> exploratory process, <u>Preface</u> exponential distributions, <u>Exponential Distributions–Exercises</u> 

guessing distribution, <u>The Estimation Game–The Estimation Game</u> guessing variance, <u>Guess the Variance–Guess the Variance</u> Kaplan-Meier, <u>Kaplan-Meier Estimation, Glossary</u> KDE, <u>Kernel Density Estimation–Kernel Density Estimation</u> linear least squares, <u>Estimation–Estimation</u> regression model parameters, <u>Estimating Parameters</u> sampling bias, <u>Sampling Bias</u> sampling distributions, <u>Sampling Distributions–Sampling Distributions</u> survival curves, <u>Estimating Survival Curves</u> survival function, <u>Estimating the Survival Function</u> estimators 

about, <u>The Estimation Game–The Estimation Game, Glossary</u> biased, <u>Guess the Variance–Sampling Distributions, Exponential Distributions–Glossary, Sampling Distributions</u> 

unbiased, <u>Guess the Variance</u> 

ethics, professional, <u>Interpretation, Reporting Results, Correlation and Causation, Time Series Analysis, Estimating the Survival Function</u> 

EWMA (exponentially-weighted moving average), <u>Moving Averages, Serial Correlation, Glossary</u> 

exogenous variables (see explanatory variables) 

expected remaining lifetime, <u>Expected Remaining Lifetime–Expected Remaining Lifetime</u> 

explanatory variables, <u>Regression–Nonlinear Relationships, Data Mining– Logistic Regression, Implementation, Glossary, Linear Regression, Prediction</u> 

exploratory data analysis 

about, <u>Exploratory Data Analysis</u> 

cleaning data, <u>Transformation</u> DataFrame data structure, <u>DataFrames–DataFrames</u> designing visualizations, <u>Other Visualizations</u> importing data, <u>Importing the Data</u> 

interpreting data, <u>Interpretation</u> 

NSFG survey, <u>The National Survey of Family Growth–The National Survey of Family Growth</u> 

statistical approach to, <u>A Statistical Approach</u> validating data, <u>Validation</u> 

variables used, <u>Variables</u> 

exponential distributions, <u>The Exponential Distribution–The Exponential Distribution, Generating Random Numbers, Exercises, PDFs, The Distribution Framework, Exponential Distributions–Exercises, Central Limit Theorem</u> 

exponentially-weighted moving average (EWMA), <u>Moving Averages, Serial Correlation, Glossary</u> 

extrapolation, <u>Extrapolation</u> 

**F** 

false negative, <u>Errors, Glossary</u> 

false positive, <u>Errors, Glossary, Autocorrelation</u> field size, <u>Comparing Percentile Ranks</u> 

FillBetween function, <u>Estimation, Prediction, Confidence Intervals</u> fillna method, <u>Missing Values, Exercises</u> 

first babies case study, Exploratory Data Analysis, <u>First Babies–First Babies, First Babies Again</u> 

FitLine function, <u>Implementation</u> fork, <u>Using the Code</u> 

frequency, <u>Distributions–Representing Histograms, Exercises, Pmfs, Glossary, Hist Implementation, Testing Proportions–Testing Proportions</u> **G** 

Gaussian distribution (see normal distribution) generative process, <u>Why Model?</u> Git repositories, <u>Using the Code</u> 

GitHub hosting service, <u>Using the Code</u> 

goodness of fit, <u>Goodness of Fit–Goodness of Fit, Glossary</u> gorilla study, <u>Sampling Distributions–Sampling Bias, Normal Distributions–Representing Normal Distributions</u> 

groupby method, <u>Characterizing Relationships, Importing and Cleaning– Importing and Cleaning, Cohort Effects, Extrapolation</u> 

**H** 

hashable types, <u>Hist Implementation</u> 

hazard function, <u>Hazard Function–The Marriage Curve, Glossary</u> HazardFunction class, <u>Hazard Function, Kaplan-Meier Estimation, Extrapolation</u> 

height, human, <u>Exercises, Relationships Between Variables–Scatter Plots</u> hexbin plots, <u>Scatter Plots</u> 

Hist object, <u>Representing Histograms–Outliers, Exercises, Pmfs–Plotting PMFs, Plotting PMFs, Hist Implementation–Hist Implementation, HypothesisTest, Testing Proportions, Kaplan-Meier Estimation</u> 

histograms, <u>Representing Histograms–Outliers, Glossary</u> 

hockey, <u>Exercises</u> 

hockey stick graph, <u>Time Series Analysis</u> 

Holm-Bonferroni method, <u>Replication</u> hypothesis testing 

about, <u>A Statistical Approach, Glossary</u> applying CLT, <u>Applying the CLT</u> chi-squared tests, <u>Chi-Squared Tests</u> choosing best test statistic, <u>Other Test Statistics–Other Test Statistics</u> classical, <u>Classical Hypothesis Testing–Classical Hypothesis Testing, Errors</u> difference in mean, <u>Testing a Difference in Means</u> effect size and, <u>Power</u> exploratory process, <u>Preface</u> first babies case study, First Babies Again HypothesisTest class, <u>HypothesisTest–First Babies Again</u> replication and, <u>Replication</u> statistically significant effect, <u>Errors</u> testing correlations, <u>Testing a Correlation</u> testing proportions, <u>Testing Proportions</u> HypothesisTest class, <u>HypothesisTest–First Babies Again, Testing a Linear Model, Exercises</u> **I** importing data, <u>Preface, Importing the Data, Importing and Cleaning– Importing and Cleaning</u> income, <u>Exercises, Prediction–Prediction</u> index column (DataFrame), <u>DataFrames, DataFrame Indexing–DataFrame Indexing, Glossary</u> inf value, <u>Expected Remaining Lifetime</u> installation considerations, <u>Using the Code</u> interarrival times, <u>The Exponential Distribution, Glossary</u> 

intercept, <u>Least Squares Fit, Implementation</u> internal classes, <u>Hist Implementation</u> interpolation, <u>Kernel Density Estimation</u> interpreting data, <u>Interpretation</u> interquartile range, <u>Percentile-Based Statistics, Glossary</u> inverse CDF, <u>Representing CDFs, Random Numbers, Glossary, Generating Random Numbers, Cdf Implementation, Sampling Distributions, Testing the CLT–Testing the CLT</u> IPython notebook, <u>Using the Code, Exercises</u> IQ scores, <u>Goodness of Fit</u> iterative solver, Estimating Parameters **J** James Joyce Ramble, <u>Comparing Percentile Ranks</u> Janert, Philipp, <u>Further Reading</u> jittering, <u>Scatter Plots, Glossary</u> join operation, <u>Data Mining–Data Mining, Glossary</u> **K** 

Kaplan-Meier estimation, <u>Kaplan-Meier Estimation, Glossary</u> KDE (kernel density estimation), <u>Kernel Density Estimation–Kernel Density Estimation, Glossary</u> kernel density estimation (KDE), <u>Kernel Density Estimation–Kernel Density Estimation, Glossary</u> **L** 

lag interval, <u>Serial Correlation, Autocorrelation–Autocorrelation, Glossary</u> least squares fit, <u>Least Squares Fit, Glossary</u> LeastSquares function, <u>Implementation</u> 

length, pregnancy (see pregnancy length) likelihood of an outcome, <u>Estimating Parameters</u> line plots, <u>Plotting PMFs</u> linear algebra, <u>Covariance</u> linear fit, <u>Preface, Glossary</u> linear least squares about, <u>Least Squares Fit</u> estimating, <u>Estimation–Estimation</u> goodness of fit, <u>Goodness of Fit–Goodness of Fit</u> implementing, <u>Implementation–Implementation</u> least squares fit, <u>Least Squares Fit–Least Squares Fit</u> residuals and, <u>Residuals</u> testing linear models, <u>Testing a Linear Model–Testing a Linear Model</u> weighted resampling, <u>Weighted Resampling–Weighted Resampling</u> linear models, testing, <u>Testing a Linear Model–Testing a Linear Model</u> linear regression, <u>Regression, Glossary, Linear Regression–Linear Regression, Prediction</u> linear relationships, <u>Nonlinear Relationships</u> linear transformation, <u>Normal Distributions</u> log odds, <u>Estimating Parameters</u> logarithm, <u>Exercises</u> logarithmic scale, <u>The Exponential Distribution</u> logistic regression, <u>Logistic Regression–Implementation, Glossary</u> logit function, <u>Implementation</u> 

lognormal distribution, <u>The lognormal Distribution–The lognormal Distribution, Exercises, Spearman’s Rank Correlation, Central Limit Theorem–Testing the CLT, Exercises</u> 

longitudinal studies, <u>The National Survey of Family Growth, Glossary</u> **M** 

MacKay, David, <u>HypothesisTest</u> marital status, <u>Exercises, Estimating Survival Curves–The Marriage Curve, Cohort Effects</u> marriage curve, <u>The Marriage Curve</u> mass, PDFs 

matplotlib package, <u>Using the Code, Plotting Histograms</u> 

maximum likelihood estimator (MLE), <u>The Estimation Game, Exponential Distributions, Glossary, Least Squares Fit, Estimating Parameters</u> 

mean 

about, <u>Summarizing Distributions, The Normal Distribution</u> computing, <u>Exercises</u> rolling, <u>Moving Averages, Glossary</u> standard scores and, <u>Correlation</u> 

testing difference in, <u>Testing a Difference in Means</u> 

mean error, <u>Guess the Variance</u> 

mean squared error (MSE), <u>The Estimation Game–The Estimation Game, Sampling Distributions, Exponential Distributions–Glossary, Goodness of Fit</u> 

MeanVar function, <u>Pearson’s Correlation</u> 

measurement error, <u>The Estimation Game, Sampling Bias, Glossary, Least Squares Fit, Estimation, Confidence Intervals</u> 

median, <u>Percentile-Based Statistics, Glossary, The Estimation Game–The Estimation Game, Expected Remaining Lifetime</u> 

memoryless processes, <u>Expected Remaining Lifetime</u> 

missing values, <u>Transformation, Testing a Correlation, Plotting, Moving Averages–Missing Values, Exercises</u> 

MLE (maximum likelihood estimator), <u>The Estimation Game, Exponential Distributions, Glossary, Least Squares Fit, Estimating Parameters</u> 

mode, <u>Summarizing Distributions, Exercises</u> 

modeling error, <u>Prediction–Prediction, Confidence Intervals</u> 

models 

about, <u>Modeling Distributions, Why Model?, Glossary</u> 

estimated PDFs, <u>Kernel Density Estimation</u> 

exponential distributions, <u>The Exponential Distribution–The Exponential Distribution</u> 

generating random numbers, <u>Generating Random Numbers–Generating Random Numbers</u> 

hypothesis testing and, <u>Classical Hypothesis Testing–Testing a Difference in Means</u> 

income distribution, <u>Exercises</u> 

least squares fit, <u>Residuals</u> 

linear regression, <u>Linear Regression–Moving Averages</u> 

lognormal distribution, <u>The lognormal Distribution, The Pareto Distribution</u> 

measuring goodness of fit, <u>Goodness of Fit–Testing a Linear Model</u> normal distribution, <u>The Normal Distribution</u> 

normal probability plot, <u>Normal Probability Plot–Normal Probability Plot, The Pareto Distribution</u> 

Pareto distribution, <u>The Pareto Distribution–The Pareto Distribution</u> regression (see regression analysis) 

relationships between variables, <u>Least Squares Fit</u> stationary, <u>Prediction, Glossary</u> 

testing linear, <u>Testing a Linear Model–Testing a Linear Model</u> 

moment of inertia, <u>Moments</u> 

moments, <u>Moments, Glossary</u> 

moving averages, <u>Moving Averages, Glossary</u> 

MSE (mean squared error), <u>The Estimation Game–The Estimation Game, Sampling Distributions, Exponential Distributions–Glossary, Goodness of Fit</u> 

multiple births, <u>Prediction</u> 

multiple regression, <u>Preface, Regression, Multiple Regression–Multiple Regression, Glossary</u> 

multiple tests, <u>Replication</u> 

**N** 

NaN (not a number), <u>Transformation, Validation, NSFG Variables, Characterizing Relationships, Testing a Correlation, Implementation, Testing a Linear Model, Data Mining, Missing Values–Serial Correlation, The Marriage Curve</u> 

National Survey of Family Growth (NSFG), <u>Preface, The National Survey of Family Growth, Other Visualizations, The Limits of PMFs, Representing CDFs, The Normal Distribution, Classical Hypothesis Testing, Testing a Difference in Means, Exercises, Data Mining</u> 

natural experiments, <u>Correlation and Causation, Glossary</u> NBUE, <u>Expected Remaining Lifetime, Glossary</u> 

Newsweek magazine, <u>Estimating the Survival Function, Cohort Effects</u> Newton’s method, <u>Estimating Parameters</u> 

noise, <u>Scatter Plots, Linear Regression, Moving Averages–Missing Values, Prediction–Prediction, The Marriage Curve</u> 

nonlinear relationships, <u>Nonlinear Relationships–Spearman’s Rank Correlation, Residuals, Nonlinear Relationships</u> 

normal distribution 

about, <u>NSFG Variables, Glossary, The Normal Distribution, Normal Distributions</u> 

estimation and, <u>The Estimation Game</u> 

for heights, <u>Exercises</u> 

normal probability plot and, <u>Normal Probability Plot–The lognormal Distribution</u> 

PDFs of, PDFs 

smoothing and, <u>The Distribution Framework</u> 

Spearman’s rank correlation and, <u>Spearman’s Rank Correlation</u> standard deviation and, <u>Correlation</u> 

variance and, <u>Guess the Variance</u> 

normal probability plot, <u>Normal Probability Plot–The lognormal Distribution, The Pareto Distribution, Glossary, Testing the CLT–Testing the CLT, Testing the CLT</u> 

normalization, <u>Pmfs, Glossary</u> 

NSFG (National Survey of Family Growth), <u>Preface, The National Survey of Family Growth, Other Visualizations, The Limits of PMFs, Representing CDFs, The Normal Distribution, Classical Hypothesis Testing, Testing a Difference in Means, Exercises, Data Mining</u> 

null hypothesis, <u>Classical Hypothesis Testing–Testing a Difference in Means, Testing a Correlation, Testing Proportions–Errors, Exercises– Glossary, Testing a Linear Model–Testing a Linear Model, Applying the CLT–Exercises</u> 

NumPy package 

about, <u>Using the Code</u> array of probabilities and, <u>Cdf Implementation</u> binning data, <u>Characterizing Relationships</u> converting date/time values, <u>Importing and Cleaning–Importing and Cleaning</u> correlation test and, <u>Correlation Test</u> covariance and, <u>Covariance</u> exploratory data analysis, <u>DataFrames, Interpretation</u> KDE and, <u>Kernel Density Estimation–Kernel Density Estimation</u> logistic regression and, <u>Implementation–Accuracy</u> normal probability plot and, <u>Normal Probability Plot</u> random numbers and, <u>DataFrame Indexing</u> scatter plots and, <u>Scatter Plots, Linear Regression</u> testing CLT, <u>Testing the CLT</u> **O** observer bias, <u>The Class Size Paradox, Exercises–Exercises</u> odds, Logistic Regression, <u>Glossary</u> OLS object, <u>StatsModels</u> one-sided test, <u>Other Test Statistics, Glossary, Applying the CLT</u> ordinary least squares, <u>Regression–StatsModels, Logistic Regression, Glossary</u> orthogonal vector, <u>Covariance</u> outliers, <u>Outliers–Outliers, Summarizing Distributions, Glossary, Skewness–Skewness, Scatter Plots–Scatter Plots, Correlation, Spearman’s Rank Correlation, The Estimation Game</u> 

oversampling, <u>The National Survey of Family Growth, Glossary, The Class Size Paradox, Weighted Resampling</u> 

**P** 

p-value 

about, <u>Preface</u> autocorrelation functions and, <u>Autocorrelation</u> computing small values, <u>Correlation Test</u> 

hypothesis testing and, <u>Classical Hypothesis Testing, HypothesisTest– Chi-Squared Tests, Errors, Replication–Glossary, Applying the CLT</u> linear models and, <u>Testing a Linear Model–Testing a Linear Model</u> linear regression and, <u>Linear Regression</u> regression analysis and, <u>StatsModels, Nonlinear Relationships, Prediction, Implementation</u> 

pandas package 

about, <u>Using the Code</u> binning data, <u>Characterizing Relationships</u> cohort effects and, <u>Cohort Effects</u> 

computing correlation, <u>Spearman’s Rank Correlation</u> computing statistics, <u>Variance</u> counting values, <u>Distributions</u> covariance and, <u>Covariance</u> 

data structures supported, <u>DataFrames–DataFrames</u> 

extrapolating survival curves, <u>Extrapolation</u> 

hazard function and, <u>Hazard Function</u> importing and cleaning data, <u>Importing and Cleaning–Importing and Cleaning</u> 

NaN values and, <u>Transformation, NSFG Variables</u> normal probability plot and, <u>Normal Probability Plot</u> regression analysis and, <u>Data Mining, Implementation</u> rolling mean and, <u>Moving Averages</u> serial correlation and, <u>Serial Correlation</u> parabolas, <u>Nonlinear Relationships</u> parameters computing sampling distribution of, <u>Testing a Linear Model</u> data compression and, <u>Why Model?</u> estimating, <u>Estimation, Logistic Regression–Estimating Parameters</u> exponential distributions, <u>The Exponential Distribution–The Exponential Distribution</u> linear regression models, <u>Regression–StatsModels</u> lognormal distribution, <u>The lognormal Distribution</u> minimizing residuals, <u>Least Squares Fit</u> normal distribution, <u>The Normal Distribution</u> Pareto distribution, <u>The Pareto Distribution</u> Pareto distribution, <u>The Pareto Distribution–The Pareto Distribution, Why Model?–Exercises, Central Limit Theorem–Testing the CLT</u> Pareto World, <u>Exercises</u> Pareto, Vilfredo, <u>The Pareto Distribution</u> Patsy syntax, <u>StatsModels, Prediction, Implementation</u> Pdf class, <u>PDFs, The Distribution Framework</u> PDFs (probability density functions) about, <u>PDFs–PDFs, Kernel Density Estimation–Kernel Density Estimation, Glossary</u> 

Cdf implementation, <u>Cdf Implementation–Cdf Implementation</u> distribution framework, <u>The Distribution Framework</u> 

Hist implementation, <u>Hist Implementation–Hist Implementation</u> 

moments and, <u>Moments</u> 

Pmf implementation, <u>Pmf Implementation</u> 

skewness and, Skewness–Skewness 

Pearson coefficient of correlation, <u>Correlation, Pearson’s Correlation– Spearman’s Rank Correlation, Testing a Correlation, Goodness of Fit, Correlation Test</u> 

Pearson median skewness, <u>Skewness–Skewness, Glossary</u> 

Pearson, Karl, <u>Pearson’s Correlation</u> 

percent point function, <u>Testing the CLT</u> 

percentile rank, <u>Percentiles–CDFs, Percentile-Based Statistics–Glossary, Correlation</u> 

percentiles, <u>Percentiles, Percentile-Based Statistics, Glossary, The Normal Distribution, Characterizing Relationships, Sampling Distributions</u> 

permutation, <u>Testing a Difference in Means, Other Test Statistics, Power, Exercises, Testing a Linear Model, Correlation Test</u> 

permutation test, <u>Other Test Statistics, Glossary</u> 

plots 

bar, <u>Plotting PMFs</u> 

hexbin, <u>Scatter Plots</u> 

line, <u>Plotting PMFs</u> 

normal probability, <u>Normal Probability Plot–The lognormal Distribution, The Pareto Distribution, Glossary, Testing the CLT–Testing the CLT, Testing the CLT</u> 

for PMFs, Plotting PMFs–Plotting PMFs 

scatter, <u>Scatter Plots–Scatter Plots, Nonlinear Relationships, Glossary, Implementation, Linear Regression</u> for time series, <u>Plotting</u> Pmf class about, <u>Pmfs–Pmfs</u> class size paradox, <u>The Class Size Paradox–The Class Size Paradox</u> computing mean and variance, <u>Exercises</u> distribution framework, <u>The Distribution Framework</u> implementing, <u>Pmf Implementation</u> KDE and, <u>Kernel Density Estimation</u> passing with, <u>Cdf Implementation</u> PDFs and, PDFs–PDFs plotting PMFs, <u>Plotting PMFs–Plotting PMFs</u> survival function and, <u>Expected Remaining Lifetime</u> PMFs (probability mass functions) about, <u>Pmfs–Pmfs, Glossary</u> computing mean, <u>Exercises</u> DataFrame indexing, <u>DataFrame Indexing–DataFrame Indexing</u> designing visualizations, <u>Other Visualizations</u> limits of, <u>The Limits of PMFs, The Limits of PMFs</u> plotting, <u>Plotting PMFs–Plotting PMFs</u> Poisson regression, <u>Logistic Regression, Exercises, Glossary</u> population, <u>The National Survey of Family Growth, Glossary, The Pareto Distribution, Sampling Distributions</u> 

power (tests), <u>Power, Glossary</u> 

prediction, <u>Pearson’s Correlation, The Estimation Game, Goodness of Fit– Goodness of Fit, Prediction–Prediction, Prediction–Prediction, Cohort Effects</u> 

preferential attachment, <u>Why Model?</u> 

pregnancy length 

applying CLT, <u>Applying the CLT</u> effect size, <u>Effect Size</u> first babies and, <u>Exploratory Data Analysis, First Babies–First Babies</u> histogram of, <u>NSFG Variables–Outliers</u> 

hypothesis testing, <u>Classical Hypothesis Testing–Classical Hypothesis Testing, Testing a Difference in Means–Other Test Statistics, First Babies Again, Power–Exercises</u> 

normal probability plot and, <u>Normal Probability Plot</u> 

plotting PMFs, <u>Plotting PMFs</u> 

regression analysis and, <u>Prediction</u> 

representing CDFs for, <u>Representing CDFs</u> 

survival analysis and, <u>Survival Curves–Hazard Function, Expected Remaining Lifetime–Expected Remaining Lifetime</u> 

variables for, <u>Variables</u> 

variance of, <u>Variance</u> 

Price of Weed website, <u>Time Series Analysis</u> probability, <u>Pmfs, Glossary, Logistic Regression</u> 

probability density, <u>PDFs, Glossary</u> 

probability density functions (PDFs) 

about, <u>PDFs–PDFs, Kernel Density Estimation–Kernel Density Estimation, Glossary</u> 

Cdf implementation, <u>Cdf Implementation–Cdf Implementation</u> 

distribution framework, <u>The Distribution Framework</u> Hist implementation, <u>Hist Implementation–Hist Implementation</u> moments and, <u>Moments</u> Pmf implementation, <u>Pmf Implementation</u> skewness and, Skewness–Skewness probability mass functions (PMFs) about, <u>Pmfs–Pmfs</u> albout, <u>Glossary</u> class size paradox, <u>The Class Size Paradox–The Class Size Paradox</u> computing mean, <u>Exercises</u> DataFrame indexing, <u>DataFrame Indexing–DataFrame Indexing</u> designing visualizations, <u>Other Visualizations</u> plotting, <u>Plotting PMFs–Plotting PMFs</u> product, <u>Exercises</u> proof by contradiction, <u>Classical Hypothesis Testing</u> properties (Python), <u>Survival Curves</u> proportions, testing, <u>Testing Proportions</u> proxy variables, <u>Prediction, Glossary</u> pseudo r-squared, <u>Implementation</u> pumpkin weight, <u>Summarizing Distributions</u> pyplot interface, <u>Plotting Histograms, Plotting PMFs, Plotting</u> **Q** quadratic model, <u>Nonlinear Relationships, Exercises</u> quantiles, <u>Percentile-Based Statistics, Glossary</u> quantizing, <u>The Distribution Framework</u> 

quartiles, <u>Percentile-Based Statistics</u> query method, <u>Survival Curves</u> quintiles, <u>Percentile-Based Statistics</u> **R** r-squared (coefficient of determination), <u>Goodness of Fit–Testing a Linear Model, Glossary, StatsModels, Multiple Regression–Prediction, Implementation</u> race, <u>Data Mining–Prediction, Implementation</u> race times, <u>Comparing Percentile Ranks</u> random module, <u>Generating Random Numbers–Exercises</u> random numbers, <u>Random Numbers, Exercises, Normal Probability Plot, Generating Random Numbers–Generating Random Numbers, Exercises, Sampling Bias</u> randomized controlled trials, <u>Correlation and Causation–Glossary</u> rank, <u>Correlation, Spearman’s Rank Correlation, Glossary</u> raw data, <u>Variables, Glossary</u> raw moments, <u>Moments, Glossary</u> recodes (variables), <u>Variables, Glossary</u> records, <u>Glossary</u> regression, <u>Regression, Glossary</u> regression analysis accuracy of, <u>Accuracy–Accuracy</u> causal relationships and, <u>Correlation and Causation</u> data mining and, <u>Data Mining–Prediction</u> estimating parameters, <u>Estimating Parameters</u> goal of, <u>Regression</u> 

implementing models, <u>Implementation</u> logistic regression, <u>Logistic Regression–Implementation</u> multiple regression, <u>Multiple Regression–Multiple Regression</u> nonlinear relationships and, <u>Nonlinear Relationships</u> prediction and, <u>Prediction–Prediction</u> StatsModels package, <u>StatsModels–StatsModels</u> RegressionResults object, <u>StatsModels</u> reindexing, <u>Moving Averages–Moving Averages, Exercises</u> relationships between variables about, <u>Relationships Between Variables</u> causation and, <u>Correlation and Causation</u> characterizing, <u>Characterizing Relationships</u> correlation and, <u>Correlation, Correlation and Causation</u> covariance and, <u>Covariance</u> modeling, <u>Least Squares Fit</u> nonlinear, <u>Nonlinear Relationships–Spearman’s Rank Correlation, Residuals</u> Pearson’s correlation, <u>Pearson’s Correlation–Spearman’s Rank Correlation</u> scatter plots, <u>Scatter Plots–Scatter Plots</u> Spearman’s rank correlation, <u>Spearman’s Rank Correlation–Exercises</u> relay races, <u>Exercises</u> replacement, <u>Random Numbers, Glossary, Scatter Plots, Power, Exercises, Estimation, Weighted Resampling</u> repositories (Git), <u>Using the Code</u> representative studies, <u>The National Survey of Family Growth, Glossary</u> 

Resample function, <u>Power</u> resampling 

about, <u>Exercises</u> 

autocorrelation function and, <u>Autocorrelation</u> chi-squared test and, <u>Chi-Squared Test</u> 

cohort effects and, <u>Cohort Effects</u> 

correlation test and, <u>Correlation Test</u> 

missing values and, <u>Missing Values</u> 

quantifying sampling error, Prediction, <u>Confidence Intervals</u> simulating experiments, <u>Estimation</u> 

weighted, <u>Weighted Resampling–Weighted Resampling, Confidence Intervals</u> 

resampling test, <u>Glossary</u> 

residuals, <u>Least Squares Fit–Residuals, Glossary, StatsModels, Linear Regression, Autocorrelation–Prediction</u> 

respondents, <u>The National Survey of Family Growth, Glossary, The lognormal Distribution</u> 

RMSE (root MSE), <u>The Estimation Game–The Estimation Game, Exponential Distributions–Glossary, Goodness of Fit, StatsModels</u> robust statistic, <u>Skewness–Skewness, Glossary, Correlation, Spearman’s Rank Correlation</u> 

rolling mean, <u>Moving Averages, Glossary</u> 

root MSE (RMSE), <u>The Estimation Game–The Estimation Game, Exponential Distributions–Glossary, Goodness of Fit, StatsModels</u> **S** 

sample mean, <u>Sampling Distributions, Exercises</u> sample median, <u>Exponential Distributions–Exercises</u> 

sample size, <u>Sampling Distributions, Exercises, Representing Normal Distributions</u> 

sample skewness, <u>Skewness, Glossary</u> sample variance, <u>Variance, Guess the Variance</u> SampleRows function, <u>Scatter Plots</u> 

samples, <u>The National Survey of Family Growth, Glossary, CDFs</u> sampling bias, <u>Sampling Bias, Glossary, Estimation</u> 

sampling distribution, <u>Sampling Distributions–Sampling Distributions, Exercises–Glossary, Estimation, Estimation, Testing a Linear Model– Weighted Resampling, Normal Distributions–Representing Normal Distributions, Applying the CLT–Correlation Test</u> 

sampling error, <u>Sampling Distributions, Glossary, Estimation, Prediction, Prediction, Confidence Intervals, Cohort Effects</u> 

sampling weight, <u>Weighted Resampling–Glossary, Confidence Intervals</u> SAT scores, <u>Goodness of Fit</u> 

saturation, <u>Scatter Plots, Glossary</u> 

scatter plots, <u>Scatter Plots–Scatter Plots, Nonlinear Relationships, Glossary, Implementation, Linear Regression</u> 

SciPy package, <u>Using the Code, The Normal Distribution, Exercises, PDFs–Kernel Density Estimation, Sampling Distributions, Correlation Test, Chi-Squared Test</u> 

seasonality, <u>Moving Averages, Missing Values, Serial Correlation</u> selection bias, <u>Exploratory Data Analysis, Exercises</u> self-selection, <u>Sampling Bias</u> sensitivity (tests), <u>Power</u> serial correlation, <u>Serial Correlation–Autocorrelation, Glossary</u> Series data structure 

about, <u>DataFrames</u> autocorrelation functions and, <u>Autocorrelation</u> computing correlation, <u>Spearman’s Rank Correlation</u> computing differences between elements, <u>Exercises</u> counting values, <u>Validation–Interpretation</u> DataFrame indexing and, <u>DataFrame Indexing</u> extrapolating survival curves, <u>Extrapolation</u> fillna method, <u>Missing Values</u> hazard function and, <u>Hazard Function</u> mapping variable names and parameters, <u>StatsModels</u> normal probability plot and, <u>Normal Probability Plot</u> NSFG variables, <u>NSFG Variables</u> rolling mean and, <u>Moving Averages</u> sex, <u>Prediction–Logistic Regression</u> sex ratio, <u>Implementation, Exercises</u> shape, <u>Comparing CDFs</u> 

significant effect (see clinically significant results; statistically significant effect) simple regression, <u>Regression, Glossary</u> simulation, <u>Kernel Density Estimation, Sampling Distributions, HypothesisTest, Estimation, Autocorrelation, Prediction, Sampling Distributions</u> 

skewness, Skewness–Glossary, <u>Correlation, Spearman’s Rank Correlation– Spearman’s Rank Correlation, Testing a Linear Model, Central Limit Theorem–Testing the CLT</u> 

slope, <u>Least Squares Fit, Implementation, Prediction, Exercises</u> 

smoothing, <u>Why Model?, The Distribution Framework, Moving Averages</u> soccer, <u>Exercises</u> 

span parameter, <u>Moving Averages, Glossary</u> Spearman coefficient of correlation, <u>Correlation, Spearman’s Rank Correlation–Exercises, Testing a Correlation</u> 

spread, <u>Summarizing Distributions, Glossary, Percentile-Based Statistics</u> spurious relationships, <u>Correlation and Causation, Multiple Regression, Glossary</u> squared residuals, <u>Least Squares Fit</u> standard deviation about, <u>Variance, Glossary, Correlation, Glossary</u> chi-squared statistic and, <u>First Babies Again</u> computing sampling distributions, <u>Exercises</u> lognormal distribution and, <u>The lognormal Distribution</u> moments and, <u>Moments</u> normal distribution and, <u>The Normal Distribution</u> normal probability plot and, <u>Normal Probability Plot–Normal Probability Plot</u> PDFs and, PDFs Pearson median skewness and, <u>Skewness</u> Pearson’s correlation and, <u>Pearson’s Correlation</u> pooled, <u>Effect Size</u> of residuals, <u>Goodness of Fit</u> RMSE and, <u>StatsModels</u> sampling distributions and, <u>Sampling Distributions–Sampling Distributions, Normal Distributions, Sampling Distributions–</u> 

<u>Representing Normal Distributions</u> standard scores and, <u>Correlation, Correlation</u> testing for difference in, <u>Other Test Statistics</u> standard error, <u>Sampling Distributions, Sampling Bias–Glossary, Estimation, Weighted Resampling, Normal Distributions, Sampling Distributions, Exercises</u> standard normal distribution, <u>Glossary, Testing the CLT</u> standard scores, <u>Correlation, Pearson’s Correlation, Glossary</u> standardized moments, <u>Skewness, Glossary</u> stationary model, <u>Prediction, Glossary</u> statistically significant effect about, <u>Classical Hypothesis Testing, Testing a Correlation, Glossary</u> correlation test and, <u>Correlation Test</u> difference in birth weight, <u>Testing a Difference in Means</u> difference in pregnancy length, <u>Testing a Difference in Means, Other Test Statistics, First Babies Again–First Babies Again</u> linear regression and, <u>Linear Regression–Linear Regression</u> multiple regression and, <u>StatsModels</u> regression analysis and, <u>Nonlinear Relationships, Prediction, Implementation–Exercises</u> replication and, <u>Replication</u> serial correlation and, <u>Serial Correlation–Autocorrelation</u> testing linear models, <u>Testing a Linear Model</u> testing proportions, <u>Testing Proportions–Chi-Squared Tests</u> threshold of, <u>HypothesisTest, Errors</u> weighted resampling and, <u>Weighted Resampling</u> 

StatsModels package, <u>Using the Code, StatsModels–StatsModels, Implementation, Exercises, Linear Regression, Autocorrelation</u> step functions, <u>CDFs, PDFs</u> Straight Dope, The, <u>Exercises</u> Student’s t-distribution, <u>Correlation Test</u> studies 

cross-sectional, <u>The National Survey of Family Growth, Glossary</u> gorilla, <u>Sampling Distributions–Sampling Bias, Normal Distributions– Representing Normal Distributions</u> longitudinal, <u>The National Survey of Family Growth, Glossary</u> representative, <u>The National Survey of Family Growth, Glossary</u> summary statistics, <u>Summarizing Distributions, Glossary, Percentile-Based Statistics</u> survival analysis about, <u>Survival Analysis, Glossary</u> cohort effects, <u>Cohort Effects–Cohort Effects</u> confidence intervals and, <u>Confidence Intervals</u> estimating survival function, <u>Estimating the Survival Function</u> expected remaining lifetime, <u>Expected Remaining Lifetime–Expected Remaining Lifetime</u> extrapolation, <u>Extrapolation</u> hazard function, <u>Hazard Function–Estimating Survival Curves</u> Kaplan-Meier estimation, <u>Kaplan-Meier Estimation</u> marriage curve, <u>The Marriage Curve</u> survival curves, <u>Survival Curves–Estimating Survival Curves</u> survival curves, <u>Survival Curves–Estimating Survival Curves, Glossary</u> 

survival rate, <u>Survival Analysis</u> SurvivalFunction class, <u>Survival Curves, Estimating the Survival Function</u> symmetric distributions, <u>Skewness, Testing a Linear Model, Applying the CLT</u> 

**T** 

tail, <u>Summarizing Distributions, Glossary, Chi-Squared Test</u> 

telephone sampling, <u>Sampling Bias</u> 

test statistic, <u>Classical Hypothesis Testing–HypothesisTest, Other Test Statistics–Other Test Statistics, Chi-Squared Tests, Glossary, Chi-Squared Test</u> 

tests 

chi-squared, <u>Chi-Squared Tests</u> 

choosing best test statistic, <u>Other Test Statistics–Other Test Statistics</u> for linear models, <u>Testing a Linear Model–Testing a Linear Model</u> multiple, <u>Replication</u> 

one-sided, <u>Other Test Statistics, Glossary, Applying the CLT</u> 

testing CLT, <u>Testing the CLT–Testing the CLT</u> 

testing correlations, <u>Testing a Correlation</u> 

testing proportions, <u>Testing Proportions</u> two-sided, <u>Other Test Statistics, Glossary, Applying the CLT</u> underpowered, <u>Power</u> 

thinkplot module 

exponential distribution and, <u>The Exponential Distribution–The Exponential Distribution</u> 

extracting height and weight, <u>Scatter Plots</u> 

hexbin plots and, <u>Scatter Plots</u> 

normal probability plot and, <u>Normal Probability Plot</u> PDFs and, PDFs–Kernel Density Estimation, <u>Skewness–Skewness</u> plotting CDFs, <u>Representing CDFs, Random Numbers</u> plotting distribution of test statistics, <u>Testing a Difference in Means</u> plotting distributions, <u>The Class Size Paradox</u> plotting fitted lines, <u>Estimation</u> plotting for time series, <u>Plotting</u> plotting histograms, <u>Plotting Histograms–NSFG Variables, First Babies</u> plotting percentiles of weight versus weight, <u>Characterizing Relationships</u> plotting PMFs, <u>Plotting PMFs–Plotting PMFs</u> survival functions and, <u>Survival Curves</u> testing CLT, <u>Testing the CLT</u> threshold, <u>HypothesisTest, Errors</u> time series about, <u>Time Series Analysis, Glossary</u> autocorrelation function and, <u>Autocorrelation–Autocorrelation</u> importing and cleaning data, <u>Importing and Cleaning–Importing and Cleaning</u> linear regression and, <u>Linear Regression–Linear Regression</u> missing values and, <u>Missing Values</u> moving averages and, <u>Moving Averages–Moving Averages</u> plotting for, <u>Plotting</u> prediction and, <u>Prediction–Prediction</u> serial correlation, <u>Serial Correlation–Autocorrelation</u> transactions, <u>Importing and Cleaning</u> 

transparency, <u>Scatter Plots</u> treatment groups, <u>Correlation and Causation–Glossary</u> trends, <u>Moving Averages</u> Trivers-Willard hypothesis, <u>Exercises</u> true negative, <u>Accuracy</u> true positive, <u>Accuracy</u> two-sided test, <u>Other Test Statistics, Glossary, Applying the CLT</u> **U** 

UBNE, <u>Expected Remaining Lifetime, Glossary</u> unbiased estimators, <u>Guess the Variance</u> underpowered tests, <u>Power</u> 

uniform distribution, <u>NSFG Variables, Glossary, Exercises, Generating Random Numbers, Scatter Plots, Correlation, Autocorrelation</u> units, <u>Correlation, Pearson’s Correlation</u> 

US Census Bureau, <u>The Pareto Distribution, Exercises</u> 

**V** 

validating data, <u>Validation</u> variables, relationships between (see relationships between variables) variance, <u>Variance, Glossary, Exercises, Guess the Variance–Guess the Variance</u> 

visualization, <u>Preface, Other Visualizations, Kernel Density Estimation, Residuals, Plotting, Extrapolation</u> **W** 

Weibull distribution, <u>Exercises</u> 

weight 

adult, <u>Relationships Between Variables–Scatter Plots, Exercises</u> 

birth (see birth weight) gorilla, <u>Sampling Distributions</u> pumpkin, <u>Summarizing Distributions</u> sampling, <u>Weighted Resampling–Glossary</u> weighted resampling, <u>Weighted Resampling–Weighted Resampling, Confidence Intervals</u> windows, <u>Moving Averages–Moving Averages, Glossary</u> wrappers, <u>Hist Implementation, Hazard Function, Sampling Distributions</u> **X** xticks command, <u>Plotting</u> 

## **Colophon** 

The animal on the cover of _Think Stats_ , second edition is an archerfish, or spinner fish ( _Toxotidae_ ). This family of fish preys on land-based insects and small animals, using their specialized mouths to shoot them down with water droplets. This family consists of seven species, which can be found from India to the Philippines, Australia, and Polynesia. 
...
The cover image is from _Dover_ . The cover fonts are URW Typewriter and Guardian Sans. The text font is Adobe Minion Pro; the heading font is Adobe Myriad Condensed; and the code font is Dalton Maag’s Ubuntu Mono. 
...
The O’Reilly logo is a registered trademarks of O’Reilly Media, Inc. _Think Stats_ , second edition, the cover image of an archerfish, and related trade dress are trademarks of O’Reilly Media, Inc. 
...
2014-10-15T19:45:13Z 

## **Think Stats** 

Table of Contents 

<u>Preface How I Wrote This Book Using the Code Contributor List Safari® Books Online How to Contact Us 1. Exploratory Data Analysis A Statistical Approach The National Survey of Family Growth Importing the Data DataFrames Variables Transformation Validation Interpretation Exercises Glossary 2. Distributions Representing Histograms Plotting Histograms NSFG Variables Outliers First Babies Summarizing Distributions Variance Effect Size Reporting Results Exercises Glossary 3. Probability Mass Functions</u> 

<u>Pmfs Plotting PMFs Other Visualizations The Class Size Paradox DataFrame Indexing Exercises Glossary</u> 

<u>4. Cumulative Distribution Functions The Limits of PMFs Percentiles CDFs Representing CDFs Comparing CDFs Percentile-Based Statistics Random Numbers Comparing Percentile Ranks Exercises Glossary</u> 

<u>5. Modeling Distributions The Exponential Distribution The Normal Distribution Normal Probability Plot The lognormal Distribution The Pareto Distribution Generating Random Numbers Why Model? Exercises Glossary</u> 

<u>6. Probability Density Functions PDFs Kernel Density Estimation The Distribution Framework</u> 

<u>Hist Implementation Pmf Implementation Cdf Implementation Moments Skewness Exercises Glossary 7. Relationships Between Variables Scatter Plots Characterizing Relationships Correlation Covariance Pearson’s Correlation Nonlinear Relationships Spearman’s Rank Correlation Correlation and Causation Exercises Glossary 8. Estimation The Estimation Game Guess the Variance Sampling Distributions Sampling Bias Exponential Distributions Exercises Glossary 9. Hypothesis Testing Classical Hypothesis Testing HypothesisTest Testing a Difference in Means Other Test Statistics Testing a Correlation</u> 

<u>Testing Proportions Chi-Squared Tests First Babies Again Errors Power Replication Exercises Glossary 10. Linear Least Squares Least Squares Fit Implementation Residuals Estimation Goodness of Fit Testing a Linear Model Weighted Resampling Exercises Glossary 11. Regression StatsModels Multiple Regression Nonlinear Relationships Data Mining Prediction Logistic Regression Estimating Parameters Implementation Accuracy Exercises Glossary 12. Time Series Analysis Importing and Cleaning</u> 

<u>Plotting Linear Regression Moving Averages Missing Values Serial Correlation Autocorrelation Prediction Further Reading Exercises Glossary 13. Survival Analysis Survival Curves Hazard Function Estimating Survival Curves Kaplan-Meier Estimation The Marriage Curve Estimating the Survival Function Confidence Intervals Cohort Effects Extrapolation Expected Remaining Lifetime Exercises Glossary 14. Analytic Methods Normal Distributions Sampling Distributions Representing Normal Distributions Central Limit Theorem Testing the CLT Applying the CLT Correlation Test Chi-Squared Test</u> 

<u>Discussion Exercises</u> 

<u>Index Colophon Copyright</u>